...
- **Última sesión:** Sesión 33 — **Entrega de Dev A del Sprint 12** (ADR-066): agentes financiero (DIAN) y de marketing. **Lo que el spec daba por hecho y no existía:** la tabla `invoices` — §1.2 deja `# Guardar en DB ...` y hace que consultar y listar facturas devuelva datos inventados, así que tres de las cuatro tools del agente financiero eran una maqueta; va en la migración 012 junto con `campaigns`. **Cuatro defectos del spec corregidos:** el dígito de verificación del NIT estaba mal (aplica los pesos al revés; comprobado contra seis NITs públicos, la versión del spec acierta uno y por casualidad, y un dígito malo hace que la DIAN rechace la factura), el dinero se sumaba en flotante (ahora centavos enteros, con un `CHECK` en la base de que el total cuadra), el IVA caía a 19% por defecto en líneas exentas, y `validate_nit` devolvía `valid: True` aunque no hubiera podido verificar nada. **Decisiones propias:** un criterio de segmentación que no se entiende da error en vez de ignorarse (si no, la campaña sale a más gente de la que el tenant eligió); las dos reglas que protegen de un envío indebido (plantilla de WhatsApp aprobada, sin repetir el mismo segmento en 24h) viven en código y no en el prompt; y el envío masivo va sin transacción abierta, con guard de reentrega para no mandarle dos veces a todo el segmento. De paso, `celery-bulk` no tenía ninguna credencial de canal y es quien envía las campañas.
...
| 11 | Webhooks Salientes & CSAT | 🟡 En progreso | Fase 2. Dev A mergeado ([PR #40](https://github.com/miguelggdev/Omnichannel-Platform/pull/40), ADR-065): modelos `TenantWebhook`/`OutgoingWebhookLog` (migración 011, con RLS; el `secret` cifrado con pgcrypto), `EventEmitter` (`app/core/events.py`), `WebhookDispatcher` (HMAC-SHA256 + control de SSRF) y las tasks `app.tasks.notification_*` con reintentos 5s/30s/300s y desactivación automática a las 10 entregas fallidas. Los 7 puntos de emisión del spec ya están cableados. Pendiente Dev B: CRUD de `/webhooks/outgoing`, CSAT completo (encuesta, endpoint de respuesta, resumen y dashboard) y, anotado en ADR-065, el RGPD/retención de `outgoing_webhook_logs.payload` |
| 12 | Agentes Financiero & Marketing | 🟡 En progreso | Fase 2. Dev A en revisión (ADR-066): nodos `financial` y `marketing` en el grafo con sus tools, tabla `invoices` (que el spec olvida) y `campaigns` en la migración 012, cliente de la DIAN con degradación local, servicio de segmentación y la task `app.tasks.bulk_execute_campaign` con throttling. Pendiente Dev B: scoring predictivo de contactos, re-ranking del RAG con cross-encoder, CRUD de `/campaigns` y endpoint de recalcular score |
