...
- **Última sesión:** Sesión 39 — **Cierre del Sprint 10 (Dev B, ADR-069)**, con lo que la Fase 2 queda completa: nodo `sentiment_analysis` en el grafo y CRUD de `/api/v1/admin/templates` sobre lo que dejó Dev A. El scoring de contactos recupera su quinto componente (sentimiento) cuando hay mensajes medidos. De paso: el listado de templates no expone el snapshot a los `admin`, los `overrides` se validan antes de encolar y el email del admin nuevo se comprueba con `auth_lookup_user()`. Antes, en la misma jornada, se mergeó el [PR #47](https://github.com/miguelggdev/Omnichannel-Platform/pull/47) (BUG-043/044/045).
- **Sesión 38** — **Revisión de bugs post-Sprint 12: fix de BUG-043, BUG-044 y BUG-045** (rama `claude/pensive-hypatia-qtusrk`): una campaña podía salir dos veces al mismo segmento (lanzamiento sin candado: doble clic en `/send`, la tool del agente llamada dos veces, o tool y CRUD a la vez) y `scheduled_for` no lo leía nadie (una campaña programada para mañana salía en el acto, y la creada por la tool con fecha no salía nunca). Ahora toda transición de estado lee la fila con `FOR UPDATE`, la task serializa los lanzamientos del tenant con un advisory lock, y `bulk_dispatch_scheduled_campaigns` (Beat, cada minuto) saca las programadas. Test de integración que reproduce el doble envío contra PostgreSQL real (falla con el código anterior). En la misma sesión, **BUG-044** (resto de la revisión): el número de factura se reserva antes de ir a la DIAN y viaja en el documento; el consecutivo sigue al mayor emitido (con `COUNT(*)` un hueco bloqueaba al tenant); los importes pasan a `BIGINT` (migración 014; en `INTEGER` una factura de más de ~21,5 M COP no se podía guardar); el filtro `metadata` usa contención JSONB (`true` ≠ `"True"`); precios no finitos y cantidades fraccionarias se rechazan; y el despachador de campañas no reencola cada minuto lo que ya está en cola (marca `SET NX EX` en Redis). Además: la tool `send_campaign` valida la plantilla de WhatsApp al confirmar, el filtro `metadata` acepta escalares guardados como texto por el CRM, y la suite de integración libera el pool en cada test (0 avisos `Exception closing connection`, antes ~320). Suite de integración completa contra PostgreSQL local con pgvector: 202 pasan, 0 fallan; mypy con la configuración del CI: 0 errores. Segunda revisión (**BUG-045**): el agente de marketing lo podía operar cualquier cliente final por WhatsApp (ahora solo `marketing.operator_contact_ids`), las tareas de Beat cross-tenant solo barrían `DEFAULT_CLIENT_ID` (migración 015, `list_active_client_ids()` `SECURITY DEFINER`), una respuesta no-JSON de la DIAN o el fallo de una tool tumbaban el turno y su reintento duplicaba efectos, y criterios de segmentación fuera de rango daban 500. Integración: 209 pasan, 0 fallan.
- **Sesión 37** — **Cierre del Sprint 12 con la mitad de Dev B** (ADR-068): re-ranking del RAG, scoring predictivo, endpoint de recalcular score y CRUD de `/campaigns`. **La decisión grande fue no meter `torch` en el proyecto:** el spec pide el cross-encoder vía `sentence-transformers` (~2,5 GB de wheel, cientos de MB de RAM por worker) en un repo que no tenía ninguna dependencia de ML local — hasta los embeddings van por la API de OpenAI. Se usa el mismo modelo exportado a ONNX (~90 MB sobre `onnxruntime`), descargado en el `docker build` y con degradación explícita: sin modelo, el RAG devuelve el orden de los embeddings en vez de fallar. La inferencia va en un executor, con un test que verifica que no bloquea el event loop. **El sentimiento sale del scoring:** pesa 0.20 en el spec pero ninguna tabla lo guarda, y el `return 50` que propone es la misma constante para todos, así que ese 20% no distinguiría a nadie; el peso se reparte entre los cuatro componentes con datos, como ya había hecho Dev A con `sentiment_avg`. **Seis defectos más del spec corregidos** (`agent_configs.settings` no existe, `Campaign.schedule` tampoco, `contact.metadata` es `metadata_`, `db.get()` sin `client_id`, `utcnow()` naive, y un `final_top_k` que duplicaba `rag_top_k`), y la task se encolaba con un solo argumento cuando recibe dos. Las dos reglas que protegen de un envío indebido pasan a tener una sola definición en `services/campaigns.py`, llamada desde los tres caminos. De paso se cerró el hallazgo del PR #42 que seguía abierto: una conversación sin contacto identificado dejaba las tools de facturas con alcance de tenant. 86 tests unitarios nuevos.
- **Sesión 36** — **Merge de la Sesión 35 a `main` y cierre de [PR #42](https://github.com/miguelggdev/Omnichannel-Platform/pull/42).** El PR había partido de `main` antes de que el PR #43 (Sprint 11) se mergeara: `git merge origin/main` sobre la rama trajo tres conflictos. `MEMORY.md`/`PROGRESS.md` tenian dos secciones "ADR-066" distintas (Sprint 11 Dev B, ya canonica en `main`, y Sprint 12 Dev A) — la de Sprint 12 pasa a **ADR-067** (tambien en `invoice_tools.py` y la migracion 013) y su "Sesion 33" local pasa a **Sesion 35** para no chocar con la que Dev B ya uso en `main`. `app/tasks/celery_app.py` se resuelve quedandose con los dos modulos de tareas nuevos (`csat_tasks`, `campaign_tasks`). Tras el merge, `ruff format` encontro 3 archivos sin formatear y N818 en `CampanaNoEnviable` (renombrada a `CampanaNoEnviableError`), y los dobles de sesion falsa de `test_invoice_tools.py`/`test_marketing_campaigns.py` no contemplaban el `execute()` extra del advisory lock ni el `.all()` de `_resolver_identificadores()` — corregidos, sin tocar produccion. **CI real destapo un problema de entorno ajeno al sprint:** `sqlalchemy[asyncio]>=2.0.25` sin techo dejo que pip resolviera 2.1.0 (liberada ese mismo dia) en el runner, tumbando a la vez el "Type Check" (el plugin `sqlalchemy.ext.mypy` que declara `pyproject.toml` ya no existe en 2.1) y el "Alembic Migration Check" (cambio el orden de DDL `CREATE TYPE`/`CREATE TABLE` para un `Enum` inline, rompiendo la migracion `001_baseline` sin relacion con este sprint) — fijado `<2.1`. Con `sqlalchemy` de vuelta en 2.0.x el plugin de mypy volvio a correr y encontro un hallazgo real: `_campana_duplicada()` recibia `session: Any`, perdiendo el tipo que da el plugin y haciendo que `.scalar_one_or_none()` se infiera `Any` contra un retorno declarado `Campaign | None` — tipado a `AsyncSession` (junto con las otras dos funciones que comparten esa sesion). Con los 9 jobs de CI en verde (incluidos Integration Tests y Docker Build), **[PR #42](https://github.com/miguelggdev/Omnichannel-Platform/pull/42) mergeado a `main`** con `--squash --delete-branch`; CI real sobre `main` tambien en verde.
- **Sesión 35** — **Entrega de Dev A del Sprint 12** (ADR-067): agentes financiero (DIAN) y de marketing. **Lo que el spec daba por hecho y no existía:** la tabla `invoices` — §1.2 deja `# Guardar en DB ...` y hace que consultar y listar facturas devuelva datos inventados, así que tres de las cuatro tools del agente financiero eran una maqueta; va en la migración 013 (renumerada de 012 tras chocar con `012_satisfaction_surveys` de Sprint 11, ya en `main`) junto con `campaigns`. **Cuatro defectos del spec corregidos:** el dígito de verificación del NIT estaba mal (aplica los pesos al revés; comprobado contra seis NITs públicos, la versión del spec acierta uno y por casualidad, y un dígito malo hace que la DIAN rechace la factura), el dinero se sumaba en flotante (ahora centavos enteros, con un `CHECK` en la base de que el total cuadra), el IVA caía a 19% por defecto en líneas exentas, y `validate_nit` devolvía `valid: True` aunque no hubiera podido verificar nada. **Decisiones propias:** un criterio de segmentación que no se entiende da error en vez de ignorarse (si no, la campaña sale a más gente de la que el tenant eligió); las dos reglas que protegen de un envío indebido (plantilla de WhatsApp aprobada, sin repetir el mismo segmento en 24h) viven en código y no en el prompt; y el envío masivo va sin transacción abierta, con guard de reentrega para no mandarle dos veces a todo el segmento. De paso, `celery-bulk` no tenía ninguna credencial de canal y es quien envía las campañas. **`/code-review` sobre el PR #42 antes del merge:** ver ADR-067 en MEMORY.md para el detalle de los hallazgos corregidos (flush antes de `expunge_all()`, métricas/eventos que se saltaban el envío masivo, resolución N+1 de identificadores, los dos guards de campaña duplicados como defensa en profundidad dentro de `campaign_tasks.py`, la carrera del consecutivo de facturas con `pg_advisory_xact_lock`, el corte de fecha con microsegundo en `list_invoices`, el drift de `CheckConstraint` entre modelo y migración, y la colisión de número de migración con el PR #43 ya mergeado).
- **Sesión 34** — CI y `/code-review` del PR #43 (Sprint 11 Dev B) antes del merge.** [PR #43](https://github.com/miguelggdev/Omnichannel-Platform/pull/43) abierto tras la Sesión 33. CI real encontró tres fallos que el entorno local (sin Docker/mypy funcional) no podía ver: drift entre el modelo `SatisfactionSurvey` (`unique=True, index=True` → un solo índice único) y la migración 012 (que creaba una constraint aparte más un índice plano — Alembic Migration Check los detecta como cambios pendientes), un error de mypy en `csat_summary` (`dict()` sobre un `Sequence[Row[...]]`), y un cast SQL roto (`:contenido::text` sin paréntesis — el patrón del repo es `CAST(:param AS tipo)` o `(:param)::tipo`) que rompía `gdpr_delete_contact()` por completo: **13 tests de integración fallando, 10 de ellos preexistentes**, no solo los 3 nuevos de este sprint. Los tres corregidos, CI en verde (9 jobs). Después, `/code-review` (8 ángulos + verificación 1-voto) encontró 9 hallazgos más, los 9 confirmados: dos severos (una encuesta CSAT expirada se podía revivir vía un link de email viejo; la redacción RGPD de `outgoing_webhook_logs.payload` borraba también lo que la empresa le había respondido al contacto, no solo lo que el contacto escribió) y siete más — ver ADR-066 en MEMORY.md para el detalle completo de los 7 corregidos y los 2 dejados a propósito (uno es un tradeoff documentado, el otro un riesgo de diseño confirmado como no explotable hoy). **Una ronda más de CI real, ya con los 7 fixes:** el propio fix del `content: null` (criterio 4 de la lista de arriba) estaba mal — `payload -> 'data' -> 'content' IS NOT NULL` no detecta un `null` de JSON, porque el operador `->` devuelve un valor `jsonb` que representa ese `null`, no un `NULL` de SQL; el test de integración nuevo lo agarró contra Postgres real. Corregido con `jsonb_typeof(...) <> 'null'`. Con eso, CI en verde completo (9 jobs) y **[PR #43](https://github.com/miguelggdev/Omnichannel-Platform/pull/43) mergeado a `main`** con `--squash --delete-branch`.
- **Sesión 33** — **Entrega de Dev B del Sprint 11** (rama `feature/sprint-11-csat-outgoing-webhooks-dev-b`, ADR-066, pendiente de PR). CRUD de configuración de webhooks salientes y el sistema de CSAT completo, sobre el engine que dejó Dev A (PR #40, ADR-065). **Un hallazgo de seguridad antes de escribir el primer test:** el path del spec (`/api/v1/webhooks/outgoing`) cae bajo `WEBHOOK_PATHS_PREFIX`, que `TenantContextMiddleware` salta sin pedir JWT — es la excepción pensada para el receptor de webhooks entrantes (firma HMAC), no para un CRUD administrativo. Con ese path, cualquiera sin sesión habría podido leer y reescribir la configuración de webhooks de cualquier tenant. Se movió a `/api/v1/outgoing-webhooks`. **CSAT:** encuesta por texto vía `deliver_message()` (mismo camino que `respond`/`human_handoff`), enganchada a `conversation.resolved` a través de `EventEmitter.on()` como preveía ADR-065; la respuesta se captura hoy solo por el link de un clic del email (`GET /api/v1/csat/respond`, público — la seguridad la da el UUID de la encuesta, no una sesión). **Repetido y atrapado en tests: el pitfall de ADR-065 sobre `shared_task` resuelto por hilo** — la primera versión del handler de CSAT tenía el mismo error que el despacho de webhooks salientes ya había corregido; un test con un broker inexistente lo agarró con un timeout real antes de commitear. **RGPD y retención**, los dos pendientes que ADR-065 dejó de este lado: `gdpr_delete_contact()` ahora redacta `outgoing_webhook_logs.payload.data.content` del contacto borrado, y `app.tasks.bulk_purge_outgoing_webhook_logs` (Beat, domingos) purga filas más viejas que 90 días. **Sin resolver, anotado:** captura de rating por texto libre en WhatsApp/Telegram (tocaría el flujo crítico de `webhook_processor.py`), botones/widget CSAT por canal, dashboard de Grafana y métricas Prometheus del §10 del spec. 58 tests unitarios nuevos + 3 de integración (estos últimos sin correr localmente por falta de Docker; verificados por CI). `ruff check`/`format` limpios; `mypy` no se pudo correr en este entorno local (Python 3.14, stub de `numpy` incompatible con `python_version=3.11` — falla igual en un archivo sin tocar de `main`, no es nuevo de esta sesión). El entorno local ademas tiene un problema preexistente y no relacionado de `pytest-asyncio`/Python 3.14 ("no current event loop") que solo aparece corriendo `pytest tests/unit` completo (reproducido también en `main` sin cambios); cada archivo tocado se verificó individualmente sin ese problema.
- **Sesión 32** — **Entrega de Dev A del Sprint 11** ([PR #40](https://github.com/miguelggdev/Omnichannel-Platform/pull/40), ADR-065): el engine de webhooks salientes completo. Dos decisiones de alcance con el usuario al arrancar: el engine va entero (reintentos y auto-desactivación incluidos, que la matriz da a Dev B pero viven dentro de la misma task) y los 7 puntos de emisión se cablean en esta entrega. **Lo que el spec no pedía y sí hacía falta:** control de SSRF — la URL la elige el tenant y el POST lo hace un worker dentro de la red privada, así que un webhook a `169.254.169.254` habría convertido la plataforma en un proxy hacia la metadata de la nube; se resuelve el host, se rechaza toda dirección no pública y no se siguen redirecciones. **Cuatro defectos del spec corregidos:** las tasks eran `async def` (Celery no espera corrutinas: habrían devuelto el coroutine sin ejecutar nada), sus nombres las enrutaban a la cola de los mensajes entrantes, `consecutive_failures` contaba intentos en vez de entregas (el umbral de 10 se habría disparado a los dos eventos y medio) y el `secret` de firma iba en claro. **Un pitfall nuevo documentado:** el proxy de `shared_task` se resuelve contra la app de Celery del hilo que lo usa — encolar desde un hilo del executor daba la app por defecto (broker `amqp://localhost`) y el mensaje no llegaba a la cola real. Verificado en CI real, 9 jobs en verde. **Revisión cruzada antes del merge (5 hallazgos, los 5 corregidos):** el POST se hacía con la transacción abierta (el mismo fallo que la revisión del PR #38 le señaló a `clone_documents()`, repetido), `consecutive_failures += 1` desde Python tenía carrera y podía dejar sin disparar la desactivación automática (ahora `UPDATE` atómico, como el fix de BUG-022), `socket.getaddrinfo()` bloqueaba el event loop del worker, una cabecera no textual del tenant (JSONB) tumbaba el intento con un `AttributeError` que se escapaba del manejo de errores —sin log, sin contadores y sin reintento— y dos menores (`max_retries` declarado sin `self.retry()`, y un `contact.updated` emitido sin cambios). Mergeado con los 9 jobs de CI en verde.
- **Sesión 31** — **Revisión cruzada y merge de la mitad de Dev A del Sprint 10** ([PR #38](https://github.com/miguelggdev/Omnichannel-Platform/pull/38), ADR-064): modelo `TenantTemplate`/`TemplateInstantiation` (migración 010, sin `client_id` ni RLS — segunda excepción a la Regla 1, misma que `clients`), `TenantCloner` (snapshot + instanciación + clonación de documentos con re-embedding) y la task `app.tasks.bulk_clone_tenant_from_template`. **Tres hallazgos de la revisión, corregidos antes del merge:** (1) el `Content-Type` de cada archivo clonado era inválido — `documents.file_type` guarda la extensión normalizada (`"pdf"`), no un MIME, y se pasaba tal cual al subir la copia; el MIME real estaba en `documents.metadata.content_type`, que el snapshot ya capturaba y nadie leía. La fixture del test daba por hecho lo contrario, por eso no se veía. (2) las copias en Storage se hacían con un `tenant_session()` abierto, reteniendo una conexión del pooler durante toda la operación; ahora se copia primero y se abre una única transacción corta para insertar las filas, igual que la subida manual. (3) la task es `acks_late=True` y una reentrega tras la caída del worker volvía a clonar desde cero: ahora si la instanciación ya tiene `target_client_id` no se re-clona. Verificado en CI real, 8 jobs en verde. **Decisión registrada:** el snapshot no clona `plan` ni los feature flags comerciales del `Client` — el tenant clonado arranca en `free` con los módulos de pago apagados.
- **Sesión 30** — **Cierre del PR #28 de Dev B.** No se rebasó: su diff contra `main` ya era duplicado de #26/#30 y, tal cual, borraba 17 archivos de #27, #31 y #32 (`phone_unification.py`, `contact_request.py`, `transcription.py`, los scripts de la migración 009 y sus tests). Lo único que aportaba sobre lo mergeado eran sus tests de integración, y de esos solo faltaba la transcripción (entrada y reposición de Webchat ya están en `test_multichannel.py`): portado a la API actual en `tests/integration/test_audio_transcription.py` (PR #33) — el audio sin texto no llega al grafo, `_guardar_transcripcion()` escribe `content` sin perder `media_url` ni `message_type`, y un tenant no toca el mensaje de otro aunque tenga su UUID. **Verificado en local contra PostgreSQL real** (pgvector/pg16 en Docker, `app_user` con `NOBYPASSRLS`, migraciones hasta 009), replicando el job `test-integration` del CI.
...
| 9 | Canales Adicionales | ✅ Completado | Fase 2. Telegram y Email (#25), Whisper (#26), unificación de contactos (#27), `/vincular` (#29), Webchat WebSocket (#30), despliegue de la migración 009 (#31), cola `media` (#32), tests de transcripción (#33). `answerCallbackQuery`/throttling de Telegram mergeado (PR #34, ADR-061). Adjuntos de email como metadata mergeado (PR #35, ADR-062). Widget JS del Webchat mergeado (PR #36, ADR-063) y fix de seguridad BUG-041 en `/auth/refresh` mergeado (PR #37). PR #28 de Dev B cerrado como duplicado |
| 10 | Templates, Clonación & Sentimiento | ✅ Completado | Fase 2. Dev A mergeado ([PR #38](https://github.com/miguelggdev/Omnichannel-Platform/pull/38), ADR-064): modelo `TenantTemplate`/`TemplateInstantiation`, `TenantCloner` y la task `bulk_clone_tenant_from_template`. Dev B (ADR-069): nodo `sentiment_analysis` en el grafo (opcional por tenant, escala con 2 mensajes muy negativos seguidos, guarda el nivel en `messages.metadata`) y CRUD de `/api/v1/admin/templates` (snapshot, listado con resumen, instanciación con overrides validados y email comprobado antes de encolar, avance). |
| 11 | Webhooks Salientes & CSAT | ✅ Completado | Fase 2. Dev A mergeado ([PR #40](https://github.com/miguelggdev/Omnichannel-Platform/pull/40), ADR-065): modelos `TenantWebhook`/`OutgoingWebhookLog` (migración 011, con RLS; el `secret` cifrado con pgcrypto), `EventEmitter` (`app/core/events.py`), `WebhookDispatcher` (HMAC-SHA256 + control de SSRF) y las tasks `app.tasks.notification_*` con reintentos 5s/30s/300s y desactivación automática a las 10 entregas fallidas. Los 7 puntos de emisión del spec ya están cableados. Dev B mergeado ([PR #43](https://github.com/miguelggdev/Omnichannel-Platform/pull/43), ADR-066): CRUD de `/api/v1/outgoing-webhooks`, CSAT completo (modelo, envío vía `deliver_message()`, respuesta de dos pasos por link de email, resumen `/admin/csat/summary`), RGPD sobre `outgoing_webhook_logs.payload` y su retención (`bulk_purge_outgoing_webhook_logs`). CI encontró y se corrigieron drift de Alembic, un error de mypy y un cast SQL roto que tumbaba 13 tests de integración; `/code-review` encontró y se corrigieron 7 hallazgos más (encuestas expiradas revivibles, redacción RGPD alcanzando mensajes salientes, link CSAT mutando por GET, entre otros — detalle en ADR-066). Pendiente sin resolver, anotado: captura de rating por texto libre en WhatsApp/Telegram (requiere tocar `webhook_processor.py`), botones/widget CSAT por canal y el dashboard de Grafana |
| 12 | Agentes Financiero & Marketing | ✅ Completado | Fase 2. Dev A mergeado ([PR #42](https://github.com/miguelggdev/Omnichannel-Platform/pull/42), ADR-067): nodos `financial` y `marketing` en el grafo con sus tools, tabla `invoices` (que el spec olvida) y `campaigns` en la migración 013 (renumerada de 012 tras chocar con `012_satisfaction_surveys` del Sprint 11, ya en `main`), cliente de la DIAN con degradación local, servicio de segmentación y la task `app.tasks.bulk_execute_campaign` con throttling. `/code-review` encontró y se corrigieron 8 hallazgos (detalle en ADR-067); CI real destapó ademas un problema de entorno ajeno al sprint (`sqlalchemy` sin techo de version resolvia 2.1.0 y rompia el plugin de mypy y el orden de DDL de Alembic — fijado a `<2.1`) y un `Any` que ocultaba un retorno mal tipado en `campaign_tasks.py`. 9 jobs de CI en verde, mergeado con `--squash --delete-branch`. Pendiente Dev B: scoring predictivo de contactos, re-ranking del RAG con cross-encoder, CRUD de `/campaigns` y endpoint de recalcular score Dev B en un PR aparte (ADR-068): re-ranking del RAG con cross-encoder sobre ONNX Runtime (no `sentence-transformers`/torch), scoring predictivo de contactos, `PUT /contacts/{id}/score` y CRUD de `/api/v1/campaigns`, con las reglas de proteccion compartidas en `services/campaigns.py`. El quinto componente del score (sentimiento) se completó con el Sprint 10 de Dev B (ADR-069). Revisión post-sprint mergeada en [PR #47](https://github.com/miguelggdev/Omnichannel-Platform/pull/47) (BUG-043/044/045). |
