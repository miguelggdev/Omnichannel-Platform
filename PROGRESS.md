...
- **Fase:** 1 — MVP Core
- **Sprint Activo:** Sprint 8 — Observabilidad, Backup & Hardening (Dev B entregado y mergeado, [PR #18](https://github.com/miguelggdev/Omnichannel-Platform/pull/18); Dev A por arrancar)
...
- [ ] Si esa entrega necesita migración, encadenarla a `007_auth_lookup_function` (`down_revision = "007_auth_lookup_function"`) para no abrir dos cabezas de Alembic
...
| 8 | Observabilidad, Backup & Hardening | 🔄 En progreso | **Hito MVP.** Dev B (PR #18): auditoría por trigger, RGPD, quick replies, backup/restore y el test e2e — 554 unitarios + 107 de integración/e2e. BUG-025 (login no funcionaba bajo RLS real) encontrado y **cerrado** con `auth_lookup_user()` SECURITY DEFINER. Falta Dev A: OpenTelemetry, métricas, dashboards y pgcrypto. El addendum de operaciones (4 features) no está asignado a nadie en la matriz |
