...
| 10 | Templates, Clonación & Sentimiento | 🟡 En progreso | Fase 2. Dev A mergeado ([PR #38](https://github.com/miguelggdev/Omnichannel-Platform/pull/38)): modelo `TenantTemplate`/`TemplateInstantiation` (migración 010, sin RLS — ADR-064), `TenantCloner` (snapshot + instanciación + clonación de documentos) y la task `bulk_clone_tenant_from_template`. Pendiente Dev B: endpoints CRUD de `/admin/templates`, nodo `sentiment_analysis` e integración en el grafo (`specs/sprint-10-templates.md`) |
