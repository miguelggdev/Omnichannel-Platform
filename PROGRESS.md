...
- [x] `tests/integration/test_audio_transcription.py` — lo único que el PR #28 aportaba sobre lo ya mergeado, portado a la API actual (`_guardar_transcripcion`, `_leer_transcripcion_previa`). El #28 se cerró en vez de rebasarse: el resto de su diff ya estaba en #26/#30 y habría borrado lo de #27, #31 y #32
...
| 9 | Canales Adicionales | ✅ Completado (revisión pendiente del PR #32) | Fase 2. Telegram y Email (#25), Whisper (#26), unificación de contactos (#27), `/vincular` (#29), Webchat WebSocket (#30), despliegue de la migración 009 (#31), cola `media` (#32). Pendiente menor: widget JS del Webchat, `answerCallbackQuery` y throttling de Telegram, adjuntos de email. PR #28 de Dev B cerrado como duplicado; lo que aportaba (tests de integración de la transcripción) va aparte |
