...
- **Sprint Activo:** Sprint 9 — Canales Adicionales. Dev A mergeado en [PR #25](https://github.com/miguelggdev/Omnichannel-Platform/pull/25); **Dev B entregado en la rama `feature/sprint-09-webchat-whisper`**, pendiente de PR y merge.
- **Última sesión:** Sesión 29 — **Entrega de Dev B del Sprint 9: Webchat (WebSocket) y transcripción con Whisper** (rama `feature/sprint-09-webchat-whisper`, 3 commits atómicos). Tres decisiones de fondo, todas por lo que el spec rompería en producción: la entrega de lo saliente va por **Redis Pub/Sub** y no por un gestor de conexiones en memoria (el socket vive en la API y la respuesta la manda un worker de Celery, con 2 réplicas de API de por medio — ADR-056); la **sesión del visitante es un token firmado por el servidor** y el `session_id`/`message_id` que manda el cliente se ignoran, porque con ids elegidos por el cliente se puede leer la conversación de otro visitante y descartarle sus mensajes por deduplicación (ADR-057); y la transcripción tiene **cola propia** (`media` + worker `celery-media`) y se interpone entre el webhook y el grafo, que lee `state["message"]["text"]` (ADR-058). Un bug propio encontrado y corregido durante la implementación: `asyncio.wait` + `gather(return_exceptions=True)` se traga la cancelación externa del cancel scope y deja la conexión colgada — reescrito con un task group de anyio. Verificado en local: **999 unitarios, 996 passed** (los 3 fallos son de `tiktoken`, que no puede descargar su encoding en esta red; en CI pasan), `ruff` y `mypy` sin errores nuevos; los 9 tests de integración nuevos los valida el CI.
- **Sesión 28 — **Arranque del Sprint 9 (Dev A): Telegram y Email** (rama `feature/sprint-09-telegram-email`, 7 commits atómicos). El spec de Sprint 9 está desactualizado en lo esencial (firmas de `MessagingProvider`, una tabla `channel_configs` y una `ProviderFactory` que no existen): se implementó sobre el patrón real (ADR-053). Decisiones de fondo por lo que rompería en producción: Telegram sin `getFile` en el webhook ni `parse_mode: HTML`, con troceo a 4096 y `update_id` como id externo; Email con TLS según el puerto (el spec usa `use_tls=True` con el 587, que falla), solo texto plano, entrada autenticada con Basic auth (el Inbound Parse no va firmado), anti-bucle contra autorrespuestas y hilo tomado del último mensaje **entrante** (ADR-055). `IgnoredWebhookError` separa los descartes legítimos de los payloads rotos (ADR-054). De paso salieron dos hallazgos en código previo: BUG-039 (el `access_token` de Meta llevaba desde el Sprint 8 a los spans de Jaeger) y BUG-040 (el endpoint leía el cuerpo entero antes de autenticar, sin tope). Verificado en local: 903 unitarios passed, `ruff`/`mypy` sin errores nuevos y 12 mutaciones deliberadas detectadas; los 12 tests de integración nuevos los valida el CI.
...
### Completado — Dev B (rama `feature/sprint-09-webchat-whisper`, sesión 29)
- [x] `app/services/messaging/webchat.py` — `WebchatProvider` sobre la ABC real: publica lo saliente en `webchat:out:{client_id}:{session_id}` (ADR-056), firma y verifica los tokens de sesión (ADR-057), `validate_signature()` rechaza siempre (el canal no entra por HTTP)
- [x] `app/api/v1/webchat.py` — `GET /api/v1/webchat/{channel_token}`: autenticación en dos niveles, reposición de mensajes perdidos al reconectar (`?session=...&last_message_id=...`), tope de frame de 16 KB, limitador de mensajes por minuto y frames de error sin traceback
- [x] `app/schemas/webchat.py` — `WebchatMessage` (entrante, sin `session_id`) y `WebchatResponse` (saliente)
- [x] `app/tasks/audio_transcription.py` — cola `media`, `getFile` para el audio de Telegram, descarga con tope real (no por `Content-Length`), Whisper y escritura de la transcripción en `messages.content` conservando `media_url`
- [x] Cola `media` en `celery_config.py` + worker `celery-media` en `docker-compose.yml` (con `JWT_SECRET`/`ENCRYPTION_KEY`, lección de BUG-026)
- [x] Cableado: `factory`, `CHANNEL_PROVIDERS`, `get_channel_config(channel, client_id)`, `Settings`, `.env.example`, router en `main.py`, `TASK_MODULES`
- [x] Tests: 95 unitarios nuevos y 9 de integración contra Postgres real (`tests/integration/test_webchat.py`)
- [x] ADR-056, ADR-057 y ADR-058 registrados en MEMORY.md

### Sin resolver en la entrega de Dev B (decisiones o trabajo aparte)
- **Adjuntos entrantes del widget:** se descartan. No hay endpoint de subida, así que un `media_url` solo puede venir del propio visitante — una URL arbitraria que después alguien descarga (la transcripción lo haría) es un SSRF servido en bandeja. Va junto con el endpoint de subida.
- **Un `channel_token` global** (`WEBCHAT_CHANNEL_TOKEN` + `DEFAULT_CLIENT_ID`), como el resto de canales, hasta que exista la tabla `channel_configs`.
- **Cada sesión de webchat es un contacto nuevo:** si el visitante borra el almacenamiento del navegador, pierde el hilo. Unificarlo exige pedirle un dato (email o teléfono), que es el mismo flujo de resolución que falta para el criterio 5 del spec.
- **El idioma de la transcripción** se registra como el configurado (`WHISPER_LANGUAGE`) o `auto`, no el que detectó Whisper: `verbose_json` lo daría, pero solo lo soporta `whisper-1` y el modelo es configurable.
...
- **Descartes:** un provider que no quiere procesar un webhook válido lanza `IgnoredWebhookError` (200 `ignored`, log `INFO`); lo roto sigue siendo `ValueError` (200 `parse_error`, con traceback). Webchat no pasa por el endpoint HTTP (es WebSocket), pero si reutiliza `parse_webhook`, aplica lo mismo.
- **`ChannelEnum` ya tiene `webchat`** y `NormalizedMessage` acepta `sender_name`. `get_channel_config("webchat")` no existe: hay que agregarlo a `CHANNEL_PROVIDERS` cuando el provider esté registrado.
...
- **Unificación de contacto entre canales (criterio 5 del spec):** un contacto por (canal, identificador). Telegram no expone el teléfono en sus mensajes y unificar por teléfono exige un flujo de resolución que no existe. `test_multichannel.py` fija el comportamiento actual.
...
| 9 | Canales Adicionales | 🔄 En revisión | Fase 2. **Dev A** ([PR #25](https://github.com/miguelggdev/Omnichannel-Platform/pull/25), mergeado): Telegram y Email sobre la ABC real. **Dev B** (rama `feature/sprint-09-webchat-whisper`, pendiente de PR): Webchat por WebSocket con entrega vía Redis Pub/Sub y sesión firmada, y transcripción de audio con Whisper en cola propia. 95 tests unitarios y 9 de integración nuevos. Sin resolver: unificación de contacto entre canales (criterio 5 del spec) y adjuntos entrantes del widget |
