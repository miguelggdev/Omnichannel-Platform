...
| 11 | Webhooks Salientes & CSAT | 🟡 En progreso (Dev B en revisión) | Fase 2. Dev A mergeado ([PR #40](https://github.com/miguelggdev/Omnichannel-Platform/pull/40), ADR-065): modelos `TenantWebhook`/`OutgoingWebhookLog` (migración 011, con RLS; el `secret` cifrado con pgcrypto), `EventEmitter` (`app/core/events.py`), `WebhookDispatcher` (HMAC-SHA256 + control de SSRF) y las tasks `app.tasks.notification_*` con reintentos 5s/30s/300s y desactivación automática a las 10 entregas fallidas. Los 7 puntos de emisión del spec ya están cableados. Dev B entregado en `feature/sprint-11-csat-outgoing-webhooks-dev-b` (ADR-066), pendiente de PR: CRUD de `/api/v1/outgoing-webhooks`, CSAT completo (modelo, envío vía `deliver_message()`, respuesta por link de email, resumen `/admin/csat/summary`), RGPD sobre `outgoing_webhook_logs.payload` y su retención (`bulk_purge_outgoing_webhook_logs`). Pendiente sin resolver, anotado: captura de rating por texto libre en WhatsApp/Telegram (requiere tocar `webhook_processor.py`), botones/widget CSAT por canal y el dashboard de Grafana |
