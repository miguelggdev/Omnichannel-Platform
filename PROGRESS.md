...
- **Fase:** 1 — MVP Core
- **Sprint Activo:** Sprint 7 — Agente de Agendamiento & CRM API (Dev B mergeado en `main`; Dev A en PR, pendiente de review/merge)
- **Coordinación Sprint 7:**
  - La migración de `service_types` (nueva, no estaba en el schema de Sprint 1 — ver `specs/sprint-07-scheduling-crm.md` §1) la crea **Dev A** cuando arranque con `calendar_tools.py`, no Dev B. Decisión del usuario 2026-09-16, para no pisarse. **Cumplido** (migración 004).
  - El Sprint 7 suma el addendum de Agent Activity Logging (`specs/sprint-07-addendum-agent-logging.md`, reprogramado desde Sprint 6 a pedido del usuario 2026-09-16): `agent_action_log.py` (modelo), `agent_logger.py` (servicio), la migración de la tabla y el middleware que envuelve los nodos del grafo (`app/agents/graph.py`/`state.py`) van con **Dev A**; los endpoints (`app/api/v1/agent_logs.py`) y sus tests van con **Dev B**. Matriz completa en METHODOLOGY.md §Sprint 7. **Cumplido** (migración 005).
- **Última actualización:** 2026-09-17
- **Última sesión:** Sesión 21 — **Revisión de bugs pedida por el usuario sobre todo el proyecto** (no solo Sprint 7): 4 agentes en paralelo (RLS/multi-tenancy, async/concurrencia, seguridad, lógica del grafo). 4 hallazgos reales en código de Sprint 7 (de la sesión 20, este mismo PR) corregidos antes de mergear: BUG-017 (traceback expuesto vía `/agent-logs`), BUG-018 (`contact_id` sin fuente legítima para el LLM en las tools de agendamiento), BUG-019 (datetimes naive en columnas `TIMESTAMPTZ` de `appointments`), BUG-020 (defensa en profundidad RLS faltante en `calendar_tools.py`/`ContactUnifier.merge()`). Detalle completo en MEMORY.md. Quedan 4 hallazgos reales **abiertos** en código preexistente (Sprint 3-6, no tocado hoy), pendientes de decisión del usuario sobre prioridad: BUG-021 (cliente Redis async cruza event loops en tareas de Celery, mismo patrón que BUG-006/011 nunca corregido para Redis), BUG-022 (race condition en `TokenBudgetGuard.record_usage`, lost update sin lock), BUG-023 (doble reserva de citas sin lock/constraint), BUG-024 (ejecución parcial de tool calls sin dejar constancia en el handoff). CI del PR #19 verificado en verde con estos fixes incluidos (9 jobs, logs reales revisados).
- **Sesión 20** — **Entrega de Dev A del Sprint 7** (branch `feature/sprint-07-scheduling`, [PR #19](https://github.com/miguelggdev/Omnichannel-Platform/pull/19)). Siete checkpoints, cada uno con tests propios y commit atómico:
  ...
  - **Total:** ~50 tests unitarios nuevos, todos verificados localmente (venv Python 3.11: ruff limpio, mypy sin errores propios — el único drift es el preexistente de `redis` en `dedup.py`, ajeno a esta entrega). CI real verificado en verde (9 jobs, logs revisados) en [PR #19](https://github.com/miguelggdev/Omnichannel-Platform/pull/19), abierto en la sesión 21 junto con los 4 fixes de bugs de esa misma sesión.
  - **Pendiente para el cierre de Sprint 7:** decidir el merge del PR #19 con el usuario.
