...
- **Última sesión:** Sesión 20 — **Entrega de Dev A del Sprint 7** (branch `feature/sprint-07-scheduling`, sin PR todavía). Siete checkpoints, cada uno con tests propios y commit atómico:
  ...
  - **Total:** ~50 tests unitarios nuevos, todos verificados localmente (venv Python 3.11: ruff limpio, mypy sin errores propios — el único drift es el preexistente de `redis` en `dedup.py`, ajeno a esta entrega). CI real (GitHub Actions) pendiente de correr al abrir el PR.
  - **Pendiente para el cierre de Sprint 7:** abrir el PR, verificar CI con logs reales (no solo el checkmark), y decidir el merge con el usuario.
