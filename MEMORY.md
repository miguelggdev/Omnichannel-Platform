...
### Hallazgo abierto: el teléfono queda en claro en `contacts.display_name`
- **Descripción:** `app/tasks/webhook_processor.py::_resolve_contact()` crea el contacto con `display_name=identifier_value`, o sea, el número de teléfono. Desde Sprint 8, `contact_identifiers.identifier_value` está cifrado — pero el mismo dato sigue en claro, y además indexado para búsqueda `ILIKE`, en la columna de al lado, hasta que un agente le pone un nombre real al contacto.
- **Por qué no se cambió en esta entrega:** el `display_name` es lo que ven los agentes en la bandeja para identificar a quién escriben; enmascararlo (últimos 4 dígitos, por ejemplo) es una decisión de producto, no un detalle técnico, y afecta a la UX del CRM entero.
- **Opciones:** (a) enmascarar al crear (`***4567`) y dejar el valor completo solo en el identificador cifrado; (b) cifrar también `display_name` y perder la búsqueda por ILIKE; (c) dejarlo como está y asumir que el teléfono es visible dentro del tenant.
- **Estado:** ABIERTO, pendiente de decisión del usuario. Mientras tanto, el cifrado del identificador sigue valiendo para el resto de canales (email, handles) y para los contactos que ya tienen nombre.
