...
### ADR-063: Widget del Webchat — un único script vanilla, sin build, servido por la API
- **Fecha:** 2026-09-22
- **Contexto:** el último pendiente de Sprint 9. El backend del Webchat (ADR-059) estaba completo, pero sin ningún cliente que hablara el protocolo, el canal era inutilizable en la práctica.
- **Decisión:**
  - **`app/static/webchat-widget.js`**: un solo archivo JS sin dependencias ni paso de build (no hay tooling de frontend en este repo — CLAUDE.md reserva Next.js para el dashboard interno de Sprint 15, que es un producto distinto; esto es el cliente del propio canal que Sprint 9 entrega, no ese dashboard). Se instala con **una sola línea**: `<script src=".../widget.js" data-token="...">`. `document.currentScript` (capturado como la primera instrucción, antes de cualquier operación async — después de eso deja de apuntar al script correcto) da el origen de la API y el token identifica el canal (no es un secreto: lo autentica la sesión firmada que emite el servidor).
  - **Aislado con Shadow DOM** (`attachShadow`): el CSS de la página del cliente no le entra al widget, ni al revés.
  - **`GET /api/v1/webchat/widget.js`** (nuevo, en `app/api/v1/webchat.py`) sirve el archivo. Mismo contenido para cualquier tenant (nada que autenticar); `<script src>` no está sujeto a CORS, así que no hace falta configurar cabeceras especiales para que cargue desde el dominio del cliente.
  - **Reconexión con backoff exponencial + jitter**, hasta 30 s. Distingue el cierre por **token inválido (4401)** u **origen no permitido (4403)** — reintentar no arregla nada, es una config de quien instaló el widget — de cualquier otro cierre (red, reinicio del servidor, inactividad), que sí reintenta.
  - **`ping` cada 20 s** (bajo margen frente a `WEBCHAT_IDLE_TIMEOUT_SECONDS`, 120 s por defecto), pausado cuando la pestaña está oculta (`visibilitychange`) para no mantener una conexión abierta por cada pestaña en segundo plano.
  - **Solo `textContent`, nunca `innerHTML`**, para el texto de cualquier mensaje: viene del servidor (la IA o un agente humano) y no debe interpretarse como HTML bajo ninguna circunstancia, sea cual sea su contenido.
  - **Persistencia mínima:** solo la sesión firmada y el último `message_id` visto, en `localStorage` (con `try/catch`: un navegador en modo privado o con el storage bloqueado no debe romper el widget, solo perder la persistencia). El historial de la conversación NO se guarda ahí — el servidor lo repone al reconectar (`webchat_history.py`); duplicarlo en el cliente sería otra fuente de verdad innecesaria.
- **Verificación (sin Python, sin CI):** este es el primer código de este proyecto que no puede probarse con `pytest`. Se construyó un arnés mínimo en Node (sin `jsdom`, DOM/`WebSocket`/`localStorage` falseados a mano, `vm.runInContext` para cargar el script real) que ejercita el protocolo de punta a punta: handshake `hello`/`connected`, deduplicación por `message_id` entre lo repuesto y lo en vivo, `ack` que limpia el estado "pendiente", un mensaje fallido cuando el socket no está abierto, reconexión sí/no según el código de cierre, botones (`button_reply`), resiliencia a un frame que no es JSON, y que el texto nunca se trata como HTML. El arnés vive en el scratchpad de la sesión, no en el repo (no hay Node en el pipeline de CI de este proyecto Python).
- **Consecuencia / límites:** es una referencia funcional y accesible, no un *design system* — colores/posición se configuran por `data-*`, no hay temas. Sin adjuntos (el canal tampoco los soporta). Sin indicador de "escribiendo..." ni acuses de lectura (el protocolo del servidor no los tiene). El texto en español está fijo en el archivo: la traducción por idioma del visitante queda fuera de esta entrega.

### ADR-064: `tenant_templates`/`template_instantiations` son la segunda excepción a la Regla 1 de CLAUDE.md (sin `client_id`, sin RLS)
- **Fecha:** 2026-09-22
- **Contexto:** Sprint 10, mitad de Dev A ("Template cloning con re-embedding", `METHODOLOGY.md` §6). Un template es un snapshot que un `super_admin` toma de un tenant origen para crear tenants nuevos a partir de él. La operación de instanciar es cross-tenant por definición: el tenant destino ni siquiera existe todavía cuando se hace la consulta, así que no hay ningún valor de `current_client_id` que tenga sentido para una política `USING (client_id = current_setting('app.current_client_id')::uuid)`.
- **Decisión:** igual que `clients` (migración 002, la primera excepción), estas dos tablas nuevas (migración 010) no llevan `client_id` ni RLS. El control de acceso es enteramente RBAC (`require_role("super_admin"/"admin")` en los endpoints de Dev B), no RLS. `source_client_id`/`target_client_id` son columnas de referencia (FK a `clients`), no de aislamiento.
- **Qué entra en el snapshot y qué no:** `agent_configs`, `quick_replies`, `tags`, metadata de `documents` (título, ruta, tipo, tamaño — nunca el embedding, que se regenera siempre por ser dependiente del modelo), el presupuesto de tokens del mes en curso y `client.settings`/`theme_config`. **Nunca** contactos, conversaciones ni mensajes — eso es dato de negocio, no configuración clonable.
- **`channel_configs` no se clona:** la spec original del sprint la incluye, pero esa tabla todavía no existe — las credenciales de canal siguen siendo globales vía `.env` (ADR-030, `DEFAULT_CLIENT_ID`) hasta que Fase 2 le dé una tabla propia por tenant. No hay nada que clonar ahí todavía.
- **El chicken-and-egg de crear un `Client` nuevo:** la política de `clients` exige `WITH CHECK (id = current_setting('app.current_client_id')::uuid)` — no se puede insertar una fila cuyo `id` no coincida con el tenant ya seteado en la transacción. `TenantCloner.instantiate()` genera el UUID del tenant nuevo en Python (`uuid4()`) *antes* de abrir `tenant_session()`, y abre la sesión con ese mismo id: el `INSERT` pasa el `WITH CHECK` porque su propio `client_id` es el que se acaba de fijar. `tests/unit/test_tenant_cloner.py::test_el_client_nuevo_usa_el_mismo_id_que_abre_tenant_session` lo fija.
- **Sin flujo de invitación todavía:** `users.password_hash` es `NOT NULL` y este repo no tiene (todavía) invitación por email ni reset de password. `instantiate()` genera un password aleatorio (`secrets.token_urlsafe`), lo hashea para la fila y devuelve el valor en claro una sola vez; la task lo deja en `template_instantiations.progress` (JSONB), visible solo a `super_admin` vía `GET /admin/templates/instantiations/{id}`. Es una solución de paso — la forma correcta es un token de invitación de un solo uso, fuera del alcance de este sprint.
- **Documentos:** se copia el archivo crudo (no el texto ya extraído en `Document.content`, que el pipeline de ingesta no usa como entrada — vuelve a descargar y extraer desde `file_url`) a una ruta nueva bajo el `client_id` del tenant nuevo, y se reencola la misma tarea de ingesta que usa una subida manual (`app.tasks.document_ingest`), no un pipeline aparte. Solo se clonan documentos en `status == "completed"`. Un archivo que ya no existe en Storage se salta con un warning, sin bloquear el resto de la clonación.
- **Nombre de la task:** `app.tasks.bulk_clone_tenant_from_template`, no `clone_tenant_from_template` de la spec — mismo motivo que `bulk_auto_close_conversations` (ADR/BUG previos): el routing automático de `celery_config.py` manda a la cola `bulk` lo que empieza por `app.tasks.bulk_*`. Agregada a `TASK_MODULES` (`celery_app.py`): sin eso el worker de `bulk` no la conoce (BUG-014).
- **Compose:** `celery-bulk` no declaraba `SUPABASE_URL`/`SUPABASE_SECRET_KEY`/`SUPABASE_STORAGE_BUCKET` — las necesita ahora para copiar archivos al clonar documentos. Agregadas junto con las demás variables del servicio.
- **Campos de la spec que no existen en este schema:** el pseudocódigo del sprint asume `AgentConfig.agent_type`/`tools_enabled`, `Document.file_name`/`file_path`/`chunk_config`, `Tag.description` y `Client.token_budget_monthly` — ninguno existe en los modelos reales de este repo. `TenantCloner` usa los campos reales (`AgentConfig.name`, `Document.file_url`/`file_type`/`file_size`/`metadata_`, `Tag.name`/`color`, y el presupuesto mensual real en la tabla `token_budgets`). Los `overrides` de `instantiate()` quedan por eso indexados por `AgentConfig.name`, no por `agent_type`.
- **Tres hallazgos de la revisión cruzada del PR #38, corregidos antes del merge:**
  1. **El Content-Type de la copia estaba mal (bug real).** `documents.file_type` no guarda un MIME: guarda la extensión normalizada (`"pdf"`, `"docx"` — ver `ALLOWED_TYPES` en `app/api/v1/documents.py`, que traduce el MIME de la subida). `clone_documents()` lo pasaba tal cual como `Content-Type` al subir la copia, así que cada archivo clonado quedaba en Storage con una cabecera inválida (`Content-Type: pdf`). El MIME real está en `documents.metadata.content_type`, que `storage_metadata()` deja al subir y que el snapshot ya capturaba pero nadie leía; ahora se usa de ahí, con `application/octet-stream` de respaldo para documentos viejos sin esa metadata. De paso, la fila clonada conserva la metadata del original con la ruta/bucket/tamaño actualizados, igual que una subida manual. El test no lo detectaba porque su fixture de `Document` daba por hecho que `file_type` era un MIME — corregido también.
  2. **Copiar archivos con una transacción abierta.** `clone_documents()` descargaba y subía cada archivo dentro de un `tenant_session()` ya abierto: con Supavisor en modo transacción eso retiene una conexión del pooler durante toda la copia, que en un tenant con muchos documentos son minutos. Ahora copia primero (un archivo por vez, sin transacción) y abre una única transacción corta al final para insertar las filas — el mismo orden que ya usaba la subida manual en `documents.py`. Un test fija el orden.
  3. **Reentrega de la task (`acks_late=True`).** Si el worker moría después de `instantiate()`, el broker reentregaba la tarea y el segundo intento creaba un tenant nuevo desde cero (UUID nuevo); lo frenaba de rebote el `UNIQUE` global de `users.email`, pero dejando un error de integridad ilegible en `error_message` y el tenant del primer intento huérfano. Ahora `_clone()` lee la instanciación antes de empezar: si ya tiene `target_client_id`, no vuelve a clonar y deja dicho qué tenant quedó a medias para que un `super_admin` decida.
- **Qué NO se clona del `Client`, a propósito:** `plan` y los feature flags comerciales (`admin_assistant_enabled`, `lead_management_enabled`, `admin_assistant_voice_enabled`) no entran en el snapshot — el tenant nuevo arranca con los defaults (`free`, todo apagado). Clonarlos sería regalar módulos de pago por el hecho de haber salido de un template de un tenant enterprise; es una decisión comercial, no de configuración.

### ADR-065: engine de webhooks salientes — firma, SSRF, reintentos y emisión de eventos
- **Fecha:** 2026-09-23
- **Contexto:** Sprint 11, mitad de Dev A ("Outgoing webhook engine (HMAC-SHA256)", `METHODOLOGY.md` §6). El tenant configura una URL suya y la plataforma le manda eventos de dominio firmados. Decisión del usuario al arrancar: el engine va completo (incluidos reintentos y desactivación automática, que en la matriz figuran del lado de Dev B pero viven dentro de la misma task) y los 7 puntos de emisión se cablean en esta entrega. Dev B se queda con el CRUD de `/webhooks/outgoing` y con el CSAT entero.
- **Firma:** HMAC-SHA256 sobre `"{timestamp}.{body}"`, no sobre el body solo — es lo que permite al receptor descartar un replay. Viaja en `X-Webhook-Signature: sha256=<hex>`. El body que se firma es exactamente el que se manda (`content=body`, nunca `json=payload`): serializar dos veces cambia el orden o los espacios y la firma deja de verificar. Las cabeceras propias se escriben **después** de las del tenant, para que un `headers` mal configurado no pueda pisar la firma.
- **SSRF (no está en el spec):** la URL la elige el tenant y quien hace el POST es un worker dentro de la red privada; sin control, un webhook a `http://169.254.169.254/...` convierte la plataforma en un proxy hacia la metadata de la nube, Redis o la propia API. `validar_destino()` exige http/https, resuelve el host y rechaza cualquier dirección no pública (privada, loopback, link-local, multicast, reservada), y el cliente va con `follow_redirects=False` (un 302 a 127.0.0.1 esquivaría el control). Queda una ventana de DNS rebinding entre la comprobación y la conexión: cerrarla exige conectar a la IP ya resuelta forzando el `Host`, lo que rompe SNI/TLS por nombre — fuera de este sprint. `OUTGOING_WEBHOOK_ALLOW_PRIVATE_HOSTS` (default `false`) existe solo para un receptor local en desarrollo.
- **El secreto va cifrado:** `tenant_webhooks.secret` es `BYTEA` con `EncryptedString` (pgcrypto), no el `VARCHAR(256)` en claro del spec. Es la credencial de firma: misma mecánica que `contact_identifiers.identifier_value` desde la migración 008, y un `SELECT` sin la clave devuelve bytes.
- **Nombre de las tasks:** `app.tasks.notification_dispatch_outgoing_webhooks` y `app.tasks.notification_send_outgoing_webhook`. Con los nombres del spec (`dispatch_outgoing_webhooks`/`send_webhook_with_retry`) el `task_routes` de `celery_config.py` las habría mandado a la cola por defecto, que es `webhooks` — la de los mensajes entrantes: un receptor externo lento habría atascado la recepción. Mismo motivo que el prefijo `bulk_` de ADR-064. Ambas en `TASK_MODULES` (BUG-014).
- **Tasks síncronas:** el spec las declara `async def`; Celery no espera corrutinas, así que esas tareas devolverían el objeto coroutine sin ejecutar nada. Como el resto de `app/tasks/*`, la tarea es síncrona y la parte async corre en `run_isolated()`.
- **Reintentos y `consecutive_failures`:** hasta 4 intentos por entrega (inmediato + 5s/30s/300s), cada uno con su propia fila en `outgoing_webhook_logs` (comparten `webhook_delivery_id`, se distinguen por `attempt`). El reintento se programa como tarea nueva con `countdown`, no con `self.retry()`: así el intento fallido queda cerrado y con su log escrito. **Desviación deliberada sobre el pseudocódigo del spec:** `consecutive_failures` cuenta *entregas* fallidas, no intentos — se incrementa solo al agotar los reintentos. Con el conteo por intento, el umbral de 10 del criterio 3 ("10 envíos fallidos") se habría disparado a los dos eventos y medio.
- **Emisión de eventos:** `EventEmitter.emit()` no envía nada, solo encola; y **nunca lanza** (criterio 11: un Redis caído no puede tumbar la respuesta a un contacto). Se emite siempre **después del commit**, como las métricas de `record_message()`: un evento dentro de la transacción anunciaría hechos que terminan en rollback. `SUPPORTED_EVENTS` vive en `app/core/events.py`, no en el CRUD de Dev B, porque lo necesitan los tres lados (quien emite, quien despacha y quien valida la suscripción). `on()` queda como enganche en proceso para el CSAT de Dev B sobre `conversation.resolved`.
- **Dos detalles del encolado que costaron depuración (pitfalls):**
  1. **El proxy de `shared_task` se resuelve por hilo.** Encolar desde un hilo del executor resolvía la tarea contra la app *por defecto* de Celery (broker `amqp://localhost`), no contra la configurada: el encolado se colgaba hasta el timeout y nunca llegaba a la cola real. `emit()` liga `apply_async` en el hilo que ya tiene la app buena y al hilo solo le manda la llamada.
  2. **El encolado va acotado a 5 segundos** (`ENQUEUE_TIMEOUT_SECONDS`) y con `retry=False`: sin eso, un broker inalcanzable bloquea a quien emite durante todo el ciclo de reintentos de conexión de kombu. El mismo riesgo existe hoy en los `.delay()` de `documents.py` y `webhook_processor.py`, que no tienen corte — queda anotado, no se tocó en esta entrega.
- **Puntos de emisión (los 7):** `message.received`, `contact.created` y `conversation.created` en `webhook_processor._process_message` (`_resolve_conversation` ahora devuelve también si creó la conversación, y `contact.created` no se emite si la unificación por teléfono fusionó el contacto recién creado dentro de uno viejo); `message.sent` en `app/agents/nodes/_delivery.py`; `conversation.resolved` en `PUT /conversations/{id}/status`; `contact.created`/`contact.updated` en los endpoints del CRM; `appointment.created` en `calendar_tools.create_appointment`.
- **`ON DELETE CASCADE` en `outgoing_webhook_logs.webhook_id`,** a diferencia de la convención del repo (`004_service_types` y siguientes no usan `ondelete`): el log no tiene sentido sin su webhook, y sin cascada el DELETE del endpoint de Dev B fallaría con violación de FK en cuanto el webhook tuviera un envío.
- **Pendiente anotado (no es de este sprint):** `outgoing_webhook_logs.payload` guarda el evento completo, que en `message.received`/`message.sent` incluye el texto del mensaje. Es dato personal: el borrado RGPD de `app/api/v1/admin.py` debería anonimizarlo (como ya hace con `audit_logs`, BUG-028) y la tabla necesita una política de retención en `maintenance.py`. Ambas cosas caen del lado de Dev B, que es quien tiene esos módulos.
- **Aislamiento:** las dos tablas son multi-tenant normales (`client_id` + FORCE RLS, migración 011) y todas las consultas del engine pasan por `tenant_session()`. `tests/integration/test_rls_all_tables.py::TestRLSOutgoingWebhooks` las cubre, incluido que el secreto no queda en claro. De paso salió del limbo el `test_tenant_webhooks` que dormía en la clase saltada de Fase 2: su INSERT ya no cuadraba con el schema real.
- **Cinco hallazgos de la revisión cruzada del PR #40, corregidos antes del merge:**
  1. **El POST se hacía con la transacción abierta — el mismo fallo que la revisión del PR #38 le señaló a `clone_documents()`.** `_enviar()` cargaba el webhook, enviaba (hasta 10s de espera al receptor) y escribía el log, todo dentro de un `tenant_session()`: con Supavisor en modo transacción eso retiene una conexión del pooler durante todo el envío, y el worker de `notifications` manda muchos en paralelo. Ahora son dos transacciones cortas — `_cargar_webhook()` antes y `_registrar_intento()` después — con el POST en medio, sin nada abierto. Los datos que necesita el envío se copian a un `_DatosDelWebhook` inmutable, porque tocar el objeto ORM con la sesión ya cerrada dispararía una carga perezosa; el dispatcher pasa a pedir un `DestinoDeWebhook` (Protocol) en vez del modelo, que cumplen los dos.
  2. **El contador de fallos tenía carrera.** `webhook.consecutive_failures += 1` es un read-modify-write desde Python, y varias entregas del mismo webhook corren a la vez en el worker: dos fallos simultáneos contaban como uno, y la desactivación automática podía no llegar nunca. Ahora es un solo `UPDATE` atómico (`consecutive_failures + 1` calculado por PostgreSQL, el apagado resuelto en el mismo `CASE`, `RETURNING` para saber cómo quedó) — misma corrección que BUG-022 en `TokenBudgetGuard.record_usage`.
  3. **Resolución DNS bloqueante en contexto async.** `validar_destino()` llamaba a `socket.getaddrinfo()`, que es síncrono: un DNS lento dejaba parado el event loop entero del worker, no solo ese envío (regla 4 de CLAUDE.md). Pasa a `async` con el `getaddrinfo` del loop, que resuelve en su propio executor.
...
### ADR-069: cierre del Sprint 10 (Dev B) — nodo de sentimiento y CRUD de templates
- **Fecha:** 2026-09-26
- **Contexto:** mitad de Dev B del Sprint 10 (`specs/sprint-10-templates.md` §2, §5-7), pendiente desde la Sesión 31. Dev A ya había dejado el modelo, `TenantCloner` y la task de clonación (ADR-064).
- **Nodo `sentiment_analysis`** (`app/agents/nodes/sentiment.py`): va entre `intent_routing` y el destino (`route_after_sentiment` → humano si hay que escalar, si no el camino de `route_after_intent`). Clasifica con `OPENAI_FALLBACK_MODEL` (gpt-4o-mini) y structured output, con los últimos 6 mensajes de contexto. **Dos `very_negative` seguidos** escalan con `handoff_reason = "negative_sentiment"` (mensaje propio en `human_handoff`). El contador `consecutive_very_negative` vive en el estado y **sobrevive entre turnos** porque el checkpointer lo persiste y `ai_processor._initial_state()` no lo pisa; se reinicia con cualquier otro nivel y al escalar.
  - **Desviaciones sobre el spec:** opcional por tenant (`"sentiment"` en `enabled_agents`, CLAUDE.md restricción 4: es una llamada extra al LLM por mensaje); un fallo del LLM **no** tumba el turno (señal secundaria: se registra y el mensaje sigue sin sentimiento, al revés que el clasificador de intents); usa `requires_handoff`/`handoff_reason` en vez de un `force_handoff`/`next_node` paralelo; el resultado se guarda en `messages.metadata.sentiment` con `metadata || {...}` en SQL, sin read-modify-write (esa columna la escribe también el proveedor).
- **Scoring con sentimiento** (`app/services/contact_scoring.py`): cierra el pendiente de ADR-068. Si el contacto tiene mensajes medidos en la ventana de 30 días, se usan los cinco pesos del spec (0.25/0.25/0.20/0.15/0.15) con el promedio de `PUNTAJE_SENTIMIENTO` (positive 100, neutral 50, negative 20, very_negative 0); si no, los cuatro pesos de siempre. AVG ignora los mensajes sin medir: no cuentan como neutrales. Un tenant que no activa el agente ve exactamente el mismo score que antes.
- **CRUD de templates** (`app/api/v1/templates.py`, montado en `/api/v1/admin/templates` — módulo propio en vez de engordar `admin.py`): alta (snapshot) y detalle solo `super_admin`; listado `super_admin` y `admin` (este solo ve públicos o de su tenant); instanciar (202 + task) y avance de la instanciación solo `super_admin`. Sin RLS en esas dos tablas (ADR-064), así que el RBAC está explícito en cada endpoint.
  - **Tres cosas que el spec no cubría:** (1) el listado nunca devuelve el snapshot — lleva prompts de sistema y rutas de documentos del tenant origen —, solo un `summary` con conteos; (2) los `overrides` se validan en el request contra una lista de campos permitidos de `agent_configs` (antes llegaban como `**kwargs` a `AgentConfig(...)` en la task y podían fijar `id`/`client_id` o tumbarla con `TypeError` minutos después del 202); (3) el email del admin se comprueba antes de encolar con `auth_lookup_user()` (ADR-046), porque `users.email` es único global y descubrirlo en el worker dejaba la instanciación en `failed`. Si el broker no acepta la task, 503 y la instanciación queda `failed`.
- **Tests:** `tests/unit/test_sentiment_analysis.py`, `tests/unit/test_templates_api.py`, ampliaciones de `test_contact_scoring.py` y `test_conversation_graph.py`, y `tests/integration/test_sprint10_dev_b.py` contra PostgreSQL real (JSONB del mensaje sin pisar la metadata del proveedor, promedio del scoring, y el flujo de templates bajo RLS con `auth_lookup_user()`).
- **Pendiente anotado:** la segmentación de campañas todavía rechaza `sentiment_avg` (Dev A lo dejó fuera por falta de datos, ADR-067); ahora hay de dónde leerlo.

### ADR-068: cierre del Sprint 12 (Dev B) — re-ranking ONNX, scoring predictivo, CRUD de campañas
- **Fecha:** 2026-09-25
- **Contexto:** mitad de Dev B del Sprint 12 sobre lo que dejó Dev A (ADR-067): re-ranking del RAG con cross-encoder (§6), scoring predictivo de contactos (§7), endpoint de recalcular score y CRUD de `/campaigns` (§4).
- **El cross-encoder NO va por `sentence-transformers`, va por ONNX Runtime.** El spec (§6.1) usa `sentence_transformers.CrossEncoder`, que arrastra `torch`: ~2,5 GB de wheel, minutos de instalación en CI y cientos de MB de RAM por worker. Este repo no tenía **ninguna** dependencia de ML local — hasta los embeddings se piden a la API de OpenAI — así que meter torch por un reranker cambia el perfil de despliegue del proyecto entero. El mismo modelo (`cross-encoder/ms-marco-MiniLM-L-6-v2`) exportado a ONNX pesa ~90 MB y corre con `onnxruntime` (~15 MB, sin torch). Misma calidad, otro runtime.
- **Degradación, no fallo, como en `dian.py`:** si falta el `.onnx`, si falta `onnxruntime` o si la sesión no carga, se registra el motivo y `rerank()` devuelve los candidatos en el orden de los embeddings. Una consulta del RAG no puede morir porque falte un archivo de modelo; el peor caso es la calidad de retrieval que ya había antes del sprint. El modelo se descarga en el `docker build`, nunca en runtime: un contenedor de producción no sale a internet a buscar pesos la primera vez que alguien pregunta algo, y si la descarga falla la imagen se construye igual.
- **La inferencia ONNX va en un executor.** `InferenceSession.run()` es síncrona y tarda decenas de ms; llamarla derecho desde una corrutina para el event loop del worker entero, no solo esa consulta (regla 4 de CLAUDE.md, y el hallazgo 3 de la revisión del PR #40 sobre `socket.getaddrinfo`). Hay un test que lo verifica con una tarea testigo: si la inferencia bloqueara, el testigo no avanzaría.
- **El sentimiento sale del scoring, no se inventa.** El spec (§7) le da peso 0.20 al sentimiento acumulado, pero **ninguna tabla lo guarda**: el nodo `sentiment_analysis` es del Sprint 10 de Dev B y sigue pendiente. El spec tapa el hueco con `return 50` ("neutral por defecto"), que no es una medida: es la misma constante para todos, así que ese 20% no distingue a nadie y solo comprime el rango útil al 80% restante. Ese 0.20 se reparte entre los cuatro componentes que sí tienen datos, manteniendo sus proporciones (0.31/0.31/0.19/0.19). Misma postura que tomó Dev A con `sentiment_avg` en la segmentación: antes que colar un valor que nadie puede auditar, se deja el hueco explícito. Cuando exista el sentimiento se vuelve a los cinco pesos y se recalcula con el endpoint; no hay nada que migrar.
- **Las métricas del score salen en un solo SELECT con subconsultas escalares,** no en los cinco viajes del pseudocódigo: hoy el endpoint recalcula de a un contacto, pero un recálculo por lotes sobre miles con cinco round-trips cada uno no se sostiene. El cálculo (`calcular_score()`) es una función pura sobre un dataclass, así que los umbrales y los pesos se testean enteros sin PostgreSQL.
- **El score se escribe con `jsonb_set`, no leyendo-modificando-escribiendo el `metadata`.** Esa columna también la edita el tenant por `PUT /contacts/{id}`, y un read-modify-write pisaría lo que otro escribiera en el medio — la lección de BUG-022 y del contador de fallos de ADR-065. El formato del valor es el que ya lee el filtro `score_min` de `segmentation.py`, con un test que lo verifica contra su regex.
- **`PUT /contacts/{id}/score` es solo para administradores** (`super_admin`, `admin`), a diferencia del resto del router de contactos, que llega hasta `agent`: el score alimenta a quien decide a quién se le manda una campaña masiva, no la atención del día a día.
- **CRUD de campañas en `/api/v1/campaigns`,** comprobado que no cae bajo `WEBHOOK_PATHS_PREFIX` — la trampa que casi deja sin autenticación al CRUD del Sprint 11. `POST` cuenta el segmento en el acto y **rechaza un criterio desconocido con 400** en vez de dejar que el worker lo descubra; `DELETE` solo en `draft`, porque una campaña enviada es el único rastro de a quién se le mandó qué.
- **`/send` commitea el `status` ANTES de encolar**, no después como el pseudocódigo: el worker de `bulk` puede levantar el mensaje apenas se publica, y vería la campaña en un estado que el endpoint está por pisar. Además encola `(client_id, campaign_id)` — el spec pasaba solo el id, con lo que la task fallaba en cuanto se ejecutaba.
- **Las dos reglas de protección pasan a tener una sola definición.** Estaban copiadas en `marketing_tools` y en `campaign_tasks`, con el comentario correcto de que importar una desde la otra habría encadenado un módulo de tasks a uno de tools de LangChain. La definición se mueve a `app/services/campaigns.py`, la capa de la que ya dependen ambos (igual que `segmentation.py`): lo que se repite sigue siendo la **llamada** en cada camino —que es lo que de verdad protege— y ahora también la hace el CRUD. De paso, la consulta de duplicados pasa de `scalar_one_or_none()` a `limit(1)`: con dos campañas previas coincidentes, perfectamente posible en una ventana de 24 h, levantaba `MultipleResultsFound` y tumbaba el envío con un error que no decía nada.
- **Seis defectos más del spec, corregidos:** `agent_configs.settings` no existe (la columna es `config`); `Campaign(schedule=...)` tampoco (es `scheduled_for`); `contact.metadata` es `metadata_`; `db.get(Campaign, id)` sin `client_id` se apoya solo en RLS, contra la convención del repo desde el PR #43; `datetime.utcnow()` es naive donde el repo usa timezone-aware; y el `final_top_k` de §6.2 no se agrega como clave nueva porque ya es `rag_top_k` — dos settings para el mismo número solo sirven para que un día no coincidan.
- **Un hallazgo de la revisión del PR #42 que seguía abierto, corregido acá:** `get_invoice_status()`/`list_invoices()` filtraban por contacto solo si `contact_id` venía en el config, así que una conversación sin contacto identificado dejaba las dos tools con alcance de **tenant** — el agente podía recitar razón social, NIT y totales de los otros clientes de la empresa. El módulo decía seguir "el mismo contrato de seguridad que `calendar_tools.py`", donde `_contact_id()` es obligatorio. Ahora el filtro es incondicional y sin contacto no se consulta nada.
- **Cobertura:** 86 tests unitarios nuevos (36 de scoring, 16 del reranker, 19 del CRUD, 3 de la fuga de facturas, más los de la regla compartida), todos sin `--run-db`. El reranker se testea entero con dobles: el `.onnx` no viaja en el repo ni se descarga en CI, y el punto es justamente que su ausencia no rompa nada.

### ADR-067: agentes financiero (DIAN) y de marketing — facturas, campañas y segmentación
- **Fecha:** 2026-09-24
- **Contexto:** Sprint 12, mitad de Dev A ("Financial agent (DIAN), Marketing agent", `METHODOLOGY.md` §6). Dos nodos nuevos en el grafo con sus tools, la persistencia que necesitan y el envío masivo. Dev B se queda con el scoring predictivo de contactos, el re-ranking del RAG con cross-encoder, el CRUD de `/campaigns` y el endpoint de recalcular score.
- **La tabla `invoices` no está en el spec, y sin ella el agente financiero es una maqueta:** §1.2 deja `# Guardar en DB ...` en `create_invoice()` y hace que `get_invoice_status()`/`list_invoices()` devuelvan datos inventados (`"invoice_number": "FE-001234"` fijo). Se agrega en la migración 012 junto con `campaigns`, las dos multi-tenant normales (`client_id` + FORCE RLS).
- **El dígito de verificación del NIT que trae el spec está mal.** §1.2 rellena el NIT por la izquierda (`zfill(15)`) y aplica los pesos en orden natural; los pesos de la DIAN van **de derecha a izquierda** (el 3 al último dígito). Comprobado contra seis NITs públicos conocidos (800197268-4, 830053812-2, 860002964-4, 890903938-8, 899999068-1, 890100577-6): la versión correcta acierta los seis, la del spec solo uno y por casualidad. Un dígito mal calculado hace que la DIAN rechace la factura. `tests/unit/test_dian.py` fija las dos cosas: que la nuestra acierta y que la del spec no.
- **Dinero en centavos enteros, no en flotante.** El spec suma `quantity * unit_price` en `float` y con IVA del 19% sobre varias líneas el total no cierra al centavo. `invoices` guarda `subtotal_cents`/`tax_total_cents`/`total_cents` como `INTEGER`, y la migración añade `CHECK (total_cents = subtotal_cents + tax_total_cents)`: una factura descuadrada no entra en la base ni por error de la aplicación.
- **El IVA deja de caer a 19% por defecto.** El spec usa `item.get("tax_rate", 0.19)` al sumar impuestos pero no al validar, así que una línea exenta sin `tax_rate` salía con 19% de IVA cobrado al cliente. Ahora la tasa es obligatoria y solo se aceptan las tres que reconoce la DIAN (0, 5, 19).
- **Sin CUFE inventado.** `DIAN_API_URL`/`DIAN_API_TOKEN` vacías (que es como está el despliegue hoy) → `consultar_contribuyente()` valida solo formato y dígito, y `enviar_factura()` lanza `DianNoConfiguradaError`: la factura queda en `pending_dian` con el motivo, y el prompt del agente le prohíbe afirmar que fue aprobada. El spec, ante un fallo, dejaba la factura en `pending_dian` pero su ejemplo de `validate_nit` devolvía `"valid": True` igual.
- **`buyer_nit` cifrado con pgcrypto,** como `contact_identifiers.identifier_value` desde la migración 008: es el documento fiscal de un tercero que la plataforma custodia por cuenta del tenant.
- **Segmentación: un criterio que no se entiende no se ignora.** `app/services/segmentation.py` lanza `CriterioInvalidoError` ante una clave desconocida. Ignorarla en silencio es la forma más fácil de mandar una campaña a más gente de la que el tenant creía haber elegido ("los VIP con score alto" → el filtro de score no existe → salen todos los VIP). Por eso `sentiment_avg`, que el spec menciona pero que todavía no tiene de dónde salir (el sentimiento por mensaje es del Sprint 10 de Dev B), da error en vez de colarse. Nunca entran en un segmento los contactos fusionados ni los borrados por RGPD.
- **El `score_min` se castea con un `CASE`, no con un `AND`.** El score lo escribe el scoring de Dev B en `contacts.metadata`, pero esa columna también la edita el tenant por la API del CRM: un `score: "alto"` haría fallar el `CAST` y con él la campaña entera. El `CASE` garantiza el orden de evaluación (un `AND` no: el planificador puede reordenar), así que solo se castea lo que ya se comprobó numérico.
- **Las dos reglas de protección de las campañas viven en código, no en el prompt** (un prompt lo puede ignorar el modelo): la plantilla de WhatsApp tiene que estar en `agent_configs.config.marketing.approved_templates` — sin integración con la API de plantillas de Meta, esa lista es la fuente de verdad, y si el tenant no declaró ninguna no sale ninguna campaña de WhatsApp, que es el lado correcto en el que equivocarse porque mandar plantillas sin aprobar hace que Meta bloquee el número — y `send_campaign()` rechaza repetir el mismo canal y los mismos criterios dentro de 24 horas.
- **Envío masivo:** `app.tasks.bulk_execute_campaign` (cola `bulk` por el prefijo del nombre, nota técnica del spec). Envía en lotes de `CAMPAIGN_MAX_MESSAGES_PER_SECOND` esperando a completar el segundo entre lotes, **sin transacción abierta** (misma lección de ADR-064/ADR-065), y persiste el avance lote a lote en transacciones cortas para que el CRUD de Dev B lo pueda mostrar en vivo. **Desviación consciente del criterio 6** ("200 mensajes tardan al menos 2 segundos"): no se duerme después del último lote, porque eso solo retrasaría el final de la campaña sin frenar ningún envío; el límite por segundo se respeta igual.
- **La campaña no escribe en `messages`:** no abre una conversación con cada contacto. Si el contacto responde, el webhook entrante crea la conversación como siempre. El rastro de la campaña son sus contadores y su `error_log`, acotado a 100 entradas (una campaña de 50.000 contactos con el canal mal configurado, si no, mete 50.000 errores en una fila JSONB).
- **`_marcar_en_envio()` es el guard de reentrega:** la task es `acks_late=True`, y una campaña que ya pasó por `sending` no se vuelve a lanzar. Sin eso, una reentrega del broker le manda el mensaje dos veces a todo el segmento.
- **Cómo se sabe si un agente está habilitado:** el spec lo mira en `state["agent_configs"]["financial"]`, clave que `ConversationState` no tiene en este repo. La fuente real es `agent_configs.config.enabled_agents`, la misma que ya decide qué intents se le ofrecen al clasificador. Sin `financial`/`marketing` ahí, el intent ni siquiera existe para ese tenant; si el nodo se alcanza igual, contesta "no habilitado" sin gastar una llamada al LLM.
- **`_agent_tools.responder_con_tools()`:** el bucle de tool calling (primera llamada → ejecutar tools → segunda llamada que redacta) estaba escrito a mano en `scheduling.py`; los dos agentes nuevos lo habrían duplicado. Vive una sola vez. `scheduling.py` no se migra en esta entrega: tiene su propio manejo de errores de Google Calendar y sus tests, y cambiarlo es refactor, no sprint.
- **Compose:** `celery-bulk` no declaraba ninguna credencial de canal y ahora es quien envía las campañas (se le agregan YCloud, Meta, Telegram y SMTP, más `CAMPAIGN_MAX_MESSAGES_PER_SECOND`), y `celery-ai` suma las variables de la DIAN porque el agente financiero corre dentro del grafo. El test de auditoría del compose incorpora la cola `bulk` a la lista de workers que envían.

### ADR-066: CRUD de webhooks salientes y CSAT completo (Sprint 11, Dev B)
- **Fecha:** 2026-09-23
- **Contexto:** mitad de Dev B de Sprint 11 sobre el engine de ADR-065: el CRUD de `/webhooks/outgoing` y el sistema de encuestas CSAT post-resolución completo (modelo, envío, respuesta, resumen), más los dos pendientes que ADR-065 dejó anotados de este lado (RGPD y retención de `outgoing_webhook_logs`).
- **El CRUD no vive en `/api/v1/webhooks/outgoing` (el path del spec), sino en `/api/v1/outgoing-webhooks`:** `TenantContextMiddleware.WEBHOOK_PATHS_PREFIX = "/api/v1/webhooks/"` salta la autenticación JWT en cualquier ruta bajo ese prefijo — es donde vive el receptor de webhooks entrantes, que se autentica por firma HMAC, no por JWT. Con el path del spec, el CRUD (que sí necesita `require_role`) habría quedado completamente sin autenticar: cualquiera podría leer y reescribir la configuración de webhooks de cualquier tenant. Se descubrió con el primer test del endpoint (401 inesperado) antes de llegar a producción.
- **El secreto solo se devuelve una vez:** `POST /outgoing-webhooks` responde con `WebhookCreateResponse`, que sí incluye `secret`; el resto de los endpoints (`GET`, `PUT`) usan `WebhookResponse`, que no lo expone. Es la misma convención que un API key: quien lo pierde tiene que rotarlo (`PUT` con un `secret` nuevo), no recuperarlo.
- **CSAT: un solo mensaje de texto por todos los canales, vía `deliver_message()`** (`app/agents/nodes/_delivery.py`, compartido con `respond`/`human_handoff`): resuelve proveedor, envía y deja el mensaje en el historial en un paso. El spec pide botones interactivos por canal (`interactive_buttons`, `email_template`, `csat_widget`); `deliver_message()` no tiene ese vocabulario y agregarlo — junto con la captura de la respuesta por botón/widget — se dejó fuera, anotado como pendiente (ver más abajo), en vez de implementado a medias.
- **Cómo se captura la respuesta hoy: solo el link de email** (`GET /api/v1/csat/respond?survey_id=&client_id=&rating=`, público, sin JWT — `PUBLIC_PATHS`). Un texto libre del contacto ("5") en WhatsApp/Telegram/Webchat **no se captura todavía**: `_resolve_conversation()` de `webhook_processor.py` no busca conversaciones `resolved` (están en `CLOSED_STATUSES`), así que esa respuesta abriría una conversación nueva en vez de encontrar la encuesta pendiente. Conectar ese camino significa tocar el flujo crítico de mensajes entrantes que comparten los demás canales; queda anotado en PROGRESS.md como pendiente en vez de apurado. `extract_rating_from_text()` (`app/services/csat.py`) ya existe, lista para cuando se conecte.
- **El link de email lleva `client_id` además de `survey_id`:** el endpoint es público (sin JWT) y `tenant_session()` necesita el tenant activo *antes* de poder leer la encuesta bajo RLS — no hay ningún `SECURITY DEFINER` (como `auth_lookup_user()`, ADR-046) para esto. La seguridad real la da `survey_id`, un UUID que nadie adivina; un `client_id` que no le corresponde simplemente no encuentra nada (404), no filtra datos de otro tenant.
- **`SatisfactionSurvey.id` se genera en Python (`uuid4()`), no se deja al `server_default`:** el link de email necesita el id de la encuesta *antes* de que la fila exista (para armarlo dentro del texto que se manda). Mismo patrón que el `contact_id`/`message_id` que ya generaba `webhook_processor.py` antes de un `flush()`.
- **Tres transacciones cortas, no una, en `send_csat_survey`:** lee la conversación (¿sigue `resolved`? ¿ya hay encuesta?) en una `tenant_session()` y la cierra, llama a `deliver_message()` (que abre y cierra la suya propia para el envío) y recién después abre una tercera para guardar la fila. Mismo hallazgo que la revisión cruzada del PR #40 le hizo a `_enviar()` de ADR-065: una sola sesión abierta durante el POST al proveedor retendría una conexión del Supavisor en modo transacción todo ese tiempo.
- **`EventEmitter.on(EVENT_CONVERSATION_RESOLVED, ...)` es el enganche, tal como preveía ADR-065**, registrado al importar `app/tasks/csat_tasks.py` — y **ese import tiene que pasar en el proceso de la API, no solo en el worker**, porque `conversation.resolved` lo emite `PUT /conversations/{id}/status`. `app/main.py` importa el módulo por su efecto (`# noqa: F401`) además de estar en `TASK_MODULES`; sin el import en `main.py`, el handler nunca se registra en el proceso que de verdad emite el evento y ninguna encuesta se programa jamás, en silencio.
- **Pitfall repetido de ADR-065, esta vez en el handler de CSAT:** la primera versión resolvía `send_csat_survey.apply_async` *dentro* de la función que corre en el hilo del executor — exactamente el error que ADR-065 ya había documentado y evitado en el despacho hardcodeado de `EventEmitter.emit()`. El test (`test_encola_con_el_delay_configurado`) lo agarró con un timeout de 5s real contra un broker inexistente antes de llegar a producción. Arreglado igual que allá: `apply_async` se liga en el hilo que ya tiene la app de Celery configurada, y al hilo solo viaja la llamada ya ligada.
- **`tests/conftest.py` gana un segundo fixture autouse (`csat_programada`)**, hermano de `eventos_emitidos`: como el handler de CSAT queda registrado globalmente en cuanto se importa `app.main` (lo que hace `api_client`/`authenticated_client` en cada test), *cualquier* test que resuelva una conversación por HTTP — no solo los de CSAT — dispara `send_csat_survey.apply_async()` de verdad. Sin este mock, `tests/unit/test_conversations.py` habría empezado a intentar hablar con Redis en cada test que cambia el status a `resolved`.
- **RGPD alcanza `outgoing_webhook_logs.payload`** (pendiente anotado en ADR-065): `gdpr_delete_contact()` ahora también llama a `_redactar_payloads_de_webhooks_salientes()`, que sobreescribe solo `payload.data.content` con `jsonb_set()` — no el payload entero — donde `payload.data.contact_id` coincide con el contacto borrado. El resto (evento, ids, timestamp) es el rastro de que el envío ocurrió, no un dato personal, y varios eventos (`conversation.resolved`, `appointment.created`) ni siquiera tienen `content`. No pasa por `audit_logs`: esa tabla no está en `AUDITED_TABLES` del trigger de la migración 006.
- **Retención de `outgoing_webhook_logs`** (el otro pendiente de ADR-065): `app.tasks.bulk_purge_outgoing_webhook_logs` (Beat, domingos 05:00 UTC — fuera de la ventana del backup diario de las 03:00) borra, tenant por tenant, las filas más viejas que `OUTGOING_WEBHOOK_LOG_RETENTION_DAYS` (90 días por defecto). Mismo patrón de recorrido por tenant que `auto_close`/`expire_old_surveys`, reusando la `_load_active_client_ids()` de `auto_close.py` en vez de duplicarla (su propio docstring dice que es "el único punto a cambiar" el día que exista un rol de servicio).
- **Cobertura:** 58 tests unitarios nuevos (18 en `test_webhooks_config.py`, 24 en `test_csat.py`, 11 en `test_csat_tasks.py`, 5 agregados a `test_maintenance.py`) contra sesiones falsas, sin `--run-db`. `tests/integration/test_audit_gdpr.py` gana `TestRgpdWebhookLogs` (3 tests) para la redacción del payload — necesita Postgres real (`pytest --run-db`), no corrido en esta sesión por falta de Docker local; queda para que lo verifique el CI.
- **No implementado en esta entrega, anotado en PROGRESS.md:** captura de rating por texto libre en WhatsApp/Telegram (requiere tocar `webhook_processor.py`), botones interactivos/widget CSAT por canal, dashboard `grafana/dashboards/csat.json` y las métricas Prometheus (`csat_score`, `csat_surveys_sent_total`, `csat_responses_total`) que el spec pide en su §10.
- **`/code-review` sobre el PR #43 antes del merge, 9 hallazgos confirmados, 7 corregidos:**
  1. **`process_response()` no comprobaba `STATUS_EXPIRED`** (solo `STATUS_RESPONDED`) — un link de email de mas de 48h revivia la encuesta y ademas inflaba `GET /admin/csat/summary`, que tampoco filtraba por status. Ahora una encuesta expirada devuelve `None`, igual que un link invalido.
  2. **`survey_exists()` sin `client_id` explicito**, unica query nueva del sprint que se apoyaba solo en RLS. Se agrega el filtro; el unico caller ya corria dentro de `tenant_session()`, asi que no era explotable hoy, pero rompia la convencion del resto del PR.
  3. **La redaccion RGPD de `outgoing_webhook_logs.payload` no distinguia `message.received` de `message.sent`**, así que un borrado de contacto tambien redactaba lo que la empresa le habia respondido — justo lo que `gdpr_delete_contact()` preserva a proposito para `messages.content` unas lineas mas arriba. Se agrega `event = 'message.received'` al `WHERE`.
  4. **El mismo `WHERE` trataba `data.content: null` igual que `data.content` ausente** (el operador `?` de JSONB solo mira la clave) — un mensaje de solo medios sin caption quedaba con el aviso de "eliminado" sin haber tenido texto nunca. Se agrega `payload -> 'data' -> 'content' IS NOT NULL`.
  5. **`GET /api/v1/csat/respond` mutaba estado** (registraba el rating) en una peticion GET publica, y el email manda 5 de esos links. Un gateway de correo o antivirus que prefetchea enlaces dispara cualquiera de los 5 antes de que el contacto abra el email, y el click real se vuelve un no-op silencioso (`process_response` ya no deja tocar una encuesta `responded`). Se parte en dos pasos: el GET arma una pagina de confirmacion sin tocar la base, y `POST /api/v1/csat/respond` es quien registra — un prefetcher no manda ese POST.
  6. **`PUT /outgoing-webhooks/{id}` con un `null` explicito para `url`/`events`/`headers`** pasaba la validacion de Pydantic (son `Optional` para que "no enviado" siga significando "no tocar") y reventaba el `flush()` con un `IntegrityError` sin atrapar — 500 en vez de 400. Se rechaza antes de tocar la base.
  7. **`send_test_webhook` armaba un `SimpleNamespace` que duplicaba `_DatosDelWebhook`** (`app/tasks/outgoing_webhooks.py`, mismos cuatro campos, mismo proposito). Se reusa el dataclass.
  - **Dos hallazgos confirmados y no corregidos, a proposito:** el registro de `EventEmitter.on()` para CSAT depende de que `app.main` importe `app.tasks.csat_tasks` (efecto de import, no una llamada explicita) — real, ya forzo el fixture `csat_programada` en `tests/conftest.py`, pero el propio verificador confirmo que hoy no es explotable (un solo entrypoint de produccion, `uvicorn app.main:create_app`); tocar `app/core/events.py` para una alternativa mas explicita queda para cuando aparezca un segundo consumidor de `.on()`. Y `send_csat_survey`'s `max_retries=2` nunca cubre un fallo de envio real (`_enviar_encuesta` lo atrapa antes) — es un tradeoff deliberado y documentado en ambos docstrings (reintentar un envio que ya pudo haber salido lo duplicaria), no un bug.
  - Ver tambien 2 hallazgos de "reuse"/"altitude" no corregidos por ser mejoras estructurales mas grandes que este PR (un registro declarativo de campos con PII para las redacciones RGPD en vez de una funcion por tabla; un helper compartido para el patron de recorrido por tenant que ya repiten `auto_close`, `csat_tasks` y `maintenance`).
...
### BUG-041: `POST /auth/refresh` no revalidaba contra la base si el usuario o su tenant seguían activos
- **Descripción:** encontrado en una auditoría general de seguridad (no en un sprint). `refresh_token()` decodificaba el JWT y reemitía un `access_token` nuevo confiando únicamente en los claims embebidos (`user_id`, `client_id`, `role`), sin volver a consultar la base. `login()` sí verifica `is_active` de usuario y tenant, pero esa verificación solo pasaba una vez, al emitir el par de tokens. Con `JWT_REFRESH_EXPIRATION_DAYS=7` por defecto, un usuario dado de baja o una organización suspendida **después** de haber iniciado sesión seguían pudiendo renovar su `access_token` indefinidamente durante esos 7 días — desactivar a alguien no le cortaba el acceso hasta que expirara el refresh_token que ya tenía.
- **Estado:** CERRADO. `_REFRESH_LOOKUP` (mismo patrón que `documents.py`/`contact_unifier.py`: WHERE explícito con `user_id` y `client_id`, además de la RLS de `tenant_session`) revalida `users.is_active` y `clients.is_active` en cada `/refresh`, antes de reemitir el `access_token`. Usuario inexistente o tenant/usuario inactivo → 401. Como `client_id` ya se conoce por el propio JWT (a diferencia del login, que todavía no lo sabe), no hace falta la función `SECURITY DEFINER` de BUG-025: una consulta normal dentro de `tenant_session()` alcanza.
- **Cómo se encontró:** revisando `tests/unit/test_auth.py::test_refresh_success`, que nunca tocaba la base — señal de que el endpoint tampoco lo hacía.

### BUG-042: `sqlalchemy[asyncio]>=2.0.25` sin techo dejó que CI resolviera una versión que rompe el repo
- **Descripción:** encontrado verificando el PR #42 (Sprint 12) en CI real, no en el código del sprint. `requirements.txt` no le pone techo a `sqlalchemy`, y el 2026-09-24 pip resolvió `2.1.0` (liberada ese mismo día) en el runner. Dos roturas simultáneas, ninguna relacionada con el sprint que se estaba revisando: (1) el job "Type Check" — `pyproject.toml` declara el plugin de mypy `sqlalchemy.ext.mypy.plugin`, que 2.1 eliminó (`No module named 'sqlalchemy.ext.mypy'`), tumbando el chequeo completo con un solo error de import antes de revisar nada; (2) el job "Alembic Migration Check" — 2.1 cambió el orden de DDL que emite `create_table()` para una columna `sa.Enum(..., name=...)` inline, y la migración `001_baseline` (sin relación alguna con Sprint 12) rompía con `type "conversation_status" does not exist` al crear `conversations`. Confirmado que no era el código del sprint: `git diff origin/main...HEAD -- migrations/versions/001_baseline.py` no mostraba ningún cambio, y las corridas de CI en `main` de ese mismo día habían pasado en verde (con `sqlalchemy` todavía resuelto en 2.0.x en el momento en que corrieron).
- **Estado:** CERRADO. `requirements.txt` fija `sqlalchemy[asyncio]>=2.0.25,<2.1`, quedándose en la versión mínima que ya exige la tabla de stack de `CLAUDE.md` ("SQLAlchemy 2.0 async") hasta evaluar el salto a 2.1 a propósito (revisar el plugin de mypy y el `create_table`/DDL antes de subir el techo).
- **Efecto colateral bueno:** con el plugin de mypy funcionando de nuevo, encontró un hallazgo real que el import roto había estado ocultando — ver el tipado `session: AsyncSession` (antes `Any`) en `_campana_duplicada()`/`_plantilla_aprobada()`/`_resolver_identificadores()` de `app/tasks/campaign_tasks.py`: con `session: Any`, `.scalar_one_or_none()` se infiere `Any` y mypy no puede comparar ese retorno contra la firma declarada (`Campaign | None`).
- **Lección para sesiones futuras:** un requirement sin techo de versión puede tumbar CI en un PR que no tocó esa dependencia — si un job falla en algo que no tiene relación aparente con el diff (una migración vieja, un plugin de una libreria de terceros), comparar contra `main`/`origin/main` antes de asumir que es un bug del sprint.

### BUG-043: Una campaña podía salir dos veces al mismo segmento, y `scheduled_for` se ignoraba
- **Descripción:** revisión de bugs post-Sprint 12 (Sesión 38). Dos defectos en el envío de campañas:
  1. **Carrera en el lanzamiento.** `_marcar_en_envio()` leía la campaña con un `SELECT` sin candado y la pasaba a `sending`. Con READ COMMITTED, dos tasks de la misma campaña leían las dos `scheduled`, las dos escribían `sending` y las dos enviaban. Dos tasks aparecían con facilidad: un doble clic en `POST /campaigns/{id}/send` (el chequeo de `draft` tampoco tenía candado), la tool `send_campaign` del agente (que ni siquiera cambiaba el estado, así que cada llamada del LLM encolaba otra), o la tool y el CRUD a la vez. El anti-duplicado de 24h tenía el mismo hueco entre dos campañas *distintas* con el mismo segmento: ninguna estaba todavía en `sending` para la otra. Reproducido contra PostgreSQL real con `tests/integration/test_campaign_concurrency.py`: con el código anterior el test falla con "la campaña salió dos veces".
  2. **`scheduled_for` no lo leía nadie.** Ni Beat ni `eta`: `POST /send` encolaba en el acto una campaña programada para mañana, y una creada por la tool con `schedule=` quedaba en `scheduled` para siempre aunque el agente le respondía "Programada para el ...".
- **Estado:** CERRADO.
  - Máquina de estados explícita: `draft` → `scheduled` (confirmación, CRUD o tool) → `sending` (solo la task, solo desde `scheduled`). `CAMPAIGN_LANZABLE` se retiró: la task ya no lanza un `draft`.
...
- **Estado:** CERRADO.
  ...
  3. Migración `014_invoice_amounts_bigint` (`ALTER COLUMN ... TYPE BIGINT`, sube y baja) y `BigInteger` en el modelo; `alembic check` limpio. **Sin orden de despliegue obligatorio:** probado el código nuevo contra la base en 013 (`INTEGER`) y todo funciona salvo las facturas de más de ~21,5 M COP, que es el bug que la 014 corrige. El `downgrade` falla a propósito si alguna factura ya supera el rango de `INTEGER`, en vez de truncar importes.
  4. Contención JSONB (`metadata @> '{"clave": valor}'`), que compara valores con su tipo y los números por valor. Para un criterio escalar no textual se acepta además el valor guardado como texto con su forma JSON (`{"nivel": 5}` encuentra `5` y `"5"`; `{"vip": true}` encuentra `true` y `"true"`), porque `contacts.metadata` también lo escribe el tenant por la API del CRM. `bool` excluido de los criterios numéricos.
...
- **También en la Sesión 38:** la tool `send_campaign` no comprobaba la plantilla de WhatsApp al confirmar (el CRUD sí): respondía "encolada" y la campaña fallaba después en el worker sin que el agente pudiera decírselo al usuario. Ahora hace el mismo chequeo. Y `tests/integration/conftest.py` libera el pool del engine al terminar cada test (`_liberar_pool_al_terminar`): los ~320 `Exception closing connection` por corrida venían de cerrar las conexiones desde el loop del test siguiente.
...
### BUG-045: El agente de marketing lo podía operar cualquier cliente final, y las tareas periódicas solo veían un tenant
- **Descripción:** segunda revisión post-Sprint 12 (Sesión 38).
  1. **Agente de marketing abierto a clientes finales (seguridad).** El nodo `marketing` vive en el mismo grafo que contesta a los clientes por WhatsApp/Instagram/etc. Habilitarlo en `enabled_agents` no decía nada de *quién* escribe: un contacto externo que pidiera "manda una promo a todos" llegaba a las tools y podía segmentar la base (y la tool le devolvía nombres de otros contactos), crear una campaña y lanzar un envío masivo a todo el tenant.
  2. **Las tareas de Beat cross-tenant solo barrían `DEFAULT_CLIENT_ID`.** `_load_active_client_ids()` hacía `SELECT id FROM clients`, la RLS lo bloquea para el rol de la app y caía al tenant por defecto (limitación documentada en ADR de Sprint 7). Con el despachador de BUG-043 pasó a ser un bug visible: las campañas programadas de cualquier otro tenant no salían nunca (tampoco su auto-cierre, expiración de CSAT ni purga de logs).
  3. **Respuesta no-JSON de la DIAN:** `dict(respuesta.json())` escapaba como `ValueError` (ni `httpx.HTTPError` ni `DianError`). Con la factura ya reservada (BUG-044), la excepción tumbaba el turno y el reintento de `ai_processor` reservaba y enviaba otra.
  4. **Fallo de una tool = turno entero caído.** En `_agent_tools.responder_con_tools()` un `ValidationError` por argumentos mal formados del LLM, o cualquier excepción de la tool, subía a `ai_processor`, que reintentaba el turno y volvía a ejecutar las tools que ya habían escrito.
  5. **Criterios de segmentación sin rango:** `last_active_days: 1e10` → `OverflowError` en `timedelta`; `NaN`/`inf` → JSON inválido para PostgreSQL; `tags` con un objeto → error de SQL. Todos daban 500 en vez de 400.
- **Estado:** CERRADO.
  1. Solo operan el agente los contactos listados en `agent_configs.config.marketing.operator_contact_ids` (p. ej. el WhatsApp del administrador). Sin lista, nadie. Se comprueba en `marketing_node()` antes de llamar al LLM y otra vez en cada tool (`_no_autorizado()`), como defensa en profundidad. El CRUD `/api/v1/campaigns` (JWT de admin) no cambia. **Efecto en despliegue:** un tenant que ya tuviera `marketing` habilitado tiene que declarar sus operadores.
  2. Migración `015_list_active_tenants_function`: `public.list_active_client_ids()`, `SECURITY DEFINER`, `STABLE`, `search_path` fijo; solo devuelve UUID de tenants activos. Mismo patrón y misma precondición abortiva que `auth_lookup_user()` (ADR-046). La RLS de `clients` queda intacta (test de integración). La caída a `DEFAULT_CLIENT_ID` se conserva solo como red para una base sin la 015, ahora con log ERROR.
  3. `dian._cuerpo_json()`: un cuerpo que no es objeto JSON es `DianError` al enviar (la factura queda pendiente con "verifica en su portal") y `None` al consultar. Además `create_invoice()` atrapa cualquier otra excepción del envío y deja la factura pendiente con "requiere revisión manual".
  4. El fallo de una tool se devuelve al modelo como resultado ("no pudo completarse (ValidationError)…"); los errores del LLM siguen propagándose a `ai_processor`.
  5. `_validar()` exige números finitos, `last_active_days` entre 0 y 36 500, `tags` solo de strings y valores de `metadata` finitos.
- **Tests:** `tests/integration/test_active_tenants.py` (falla con el código anterior: el tenant no por defecto no aparece) y unitarios de operador de marketing, tools caídas, DIAN no-JSON y criterios fuera de rango.
