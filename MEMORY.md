...
### ADR-056: Webchat entrega lo saliente por Redis Pub/Sub, no por un gestor de conexiones en memoria
- **Fecha:** 2026-09-20
- **Contexto:** `specs/sprint-09-channels.md` §2 construye `WebchatProvider(provider_config, connection_manager)` con un `dict` `session_id -> WebSocket` en memoria. Ese diseño solo funciona si quien envía comparte proceso con el socket, y en esta arquitectura **nunca** lo comparte: el socket vive en el proceso de la API y la respuesta del agente la genera y la envía un worker de Celery (`ai_inference`, `deliver_message()`). Además, `docker-compose.yml` levanta la API con **2 réplicas** detrás de Traefik, así que ni siquiera dos conexiones de la misma API comparten memoria.
- **Decisión:** `WebchatProvider.send_message()` publica el frame en el canal de Redis `webchat:out:{client_id}:{session_id}`; el endpoint WebSocket está suscrito a ese canal y reenvía al socket. La factory sigue construyendo sin argumentos (ADR-053). El `client_id` forma parte del nombre del canal, así que `get_channel_config()` gana un parámetro `client_id` opcional (hoy solo lo usa webchat; cuando exista `channel_configs` toda la config será por tenant).
- **Consecuencia:** que no haya suscriptores no es un error — el visitante cerró la pestaña. El mensaje ya quedó persistido y se le repone al reconectar. Redis pasa a ser parte del camino de respuesta de webchat, no solo de la deduplicación: si Redis está caído, el mensaje se guarda pero no se entrega en vivo (se recupera al reconectar).

### ADR-057: La sesión de webchat es un token firmado por el servidor, no un `session_id` que elige el cliente
- **Fecha:** 2026-09-20
- **Contexto:** en el spec, el widget manda su `session_id` en la URL y en cada frame. El `session_id` es el identificador de contacto del canal, así que quien lo diga es de quien se considera el mensaje — y al reconectar con `last_message_id` se le entrega el historial de esa sesión. Con un id que elige el cliente, adivinar (o ver en el HTML de otro) un `session_id` basta para leer la conversación de otro visitante y escribir en su nombre. Es el único canal donde el identificador no lo emite un tercero de confianza (WhatsApp, Meta, Telegram).
- **Decisión:** el servidor emite `session_id = uuid4().hex` y devuelve `"<session_id>.<HMAC-SHA256>"`, firmado con `JWT_SECRET` y una etiqueta de dominio (`webchat-session:`) para que esa firma no sea intercambiable con la de un token de acceso. La verificación es con `compare_digest` sobre el token completo. Un token inválido no cierra la conexión: se le asigna una sesión nueva. El `session_id` y el `message_id` que mande el cliente en el cuerpo del frame **se ignoran**: la sesión es la del socket y el `external_message_id` lo genera el servidor (si lo eligiera el cliente podría registrar el id de otro y hacer que el mensaje ajeno se descartara como duplicado — la clave de `webhook_dedup` es global por canal).
- **Consecuencia:** el `channel_token` de la URL autentica la **instalación** del widget (viaja en el HTML de la página: no es un secreto fuerte), y el token de sesión autentica al **visitante**. Los dos niveles son distintos a propósito. `validate_signature()` del provider devuelve `False` siempre, para cerrar el camino HTTP: webchat no entra por `POST /api/v1/webhooks/`.

### ADR-058: La transcripción de audio tiene cola propia (`media`) y se interpone entre el webhook y el grafo
- **Fecha:** 2026-09-20
- **Contexto:** el spec pide `queue="media"`, que no existía. La alternativa era reusar `documents`, la otra cola que hace I/O pesado contra OpenAI. Ahí viven la ingesta y el OCR de documentos, que tardan minutos y corren con concurrencia 2: una nota de voz de tres segundos esperaría detrás de un PDF con el contacto mirando la pantalla. Además había que decidir dónde se interpone la transcripción: el grafo lee `state["message"]["text"]`, así que un audio sin transcribir le llega vacío.
- **Decisión:** cola `media` con su worker en `docker-compose.yml` (`celery-media`, concurrencia 2) y routing por nombre (`app.tasks.media_*`). `webhook_processor` guarda el mensaje de audio y, en vez de encolar la IA, encola `app.tasks.media_transcribe_audio`; la tarea descarga, transcribe, escribe `messages.content` (conservando `media_url`) y encola la IA con el texto ya puesto en el `NormalizedMessage`. Un audio que **sí** trae texto (el pie de un audio de WhatsApp) no se transcribe: ya hay con qué responder.
- **Consecuencia:** si la transcripción falla definitivamente, la IA se encola igual con `TEXTO_SIN_TRANSCRIBIR`: el contacto escribió y merece respuesta, aunque sea para pedirle que lo repita por escrito. Un audio demasiado grande no reintenta (descargarlo otra vez da el mismo tamaño). El worker `celery-media` necesita `TELEGRAM_CHANNEL_BOT_TOKEN`, porque el audio de Telegram se resuelve con `getFile` y esa URL lleva el token — que no se loguea ni se guarda.
