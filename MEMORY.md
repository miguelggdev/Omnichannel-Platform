...
### ADR-043: El login busca al usuario con una función `SECURITY DEFINER`
- **Fecha:** 2026-09-17
- **Contexto:** BUG-017. Autenticar es la única operación del sistema que necesita leer `users` sin saber a qué tenant pertenece la fila: el tenant se deduce del usuario, y el usuario es justo lo que se busca. La RLS lo impide por diseño, así que hacía falta una excepción explícita. Las tres salidas estaban planteadas en BUG-017; el usuario eligió la (a).
- **Decisión:** `public.auth_lookup_user(p_email text)` (migración 005), `SECURITY DEFINER`, `STABLE`, con `SET search_path = pg_catalog, public, pg_temp` y los nombres calificados. Devuelve una sola fila —la del email exacto— con lo que el login necesita, incluido `client_is_active`, porque `clients` también tiene RLS y consultarla aparte chocaría con lo mismo. `app/api/v1/auth.py` la llama y nada más la llama.
...
### BUG-017: El login no funcionaba contra un rol sujeto a RLS
...
- **Estado:** CERRADO 2026-09-17, con la salida (a) de las tres que se plantearon — decisión del usuario. `public.auth_lookup_user()` (migración 005) es ahora el único punto del sistema que lee `users`/`clients` sin contexto de tenant, y solo sabe resolver un email exacto. Ver ADR-043 para el razonamiento completo y la precondición de `BYPASSRLS` que la migración comprueba.
...
- **Ahora desbloqueado:** la migración 004 dejó `users` sin trigger de auditoría por este bug. Con el login arreglado, agregarlo es viable — pero exige antes auditar que **todos** los caminos de escritura sobre `users` pasen por `tenant_session()`, no solo el del login. Queda como tarea aparte, no se coló en el arreglo.
