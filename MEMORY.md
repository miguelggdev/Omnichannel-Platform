...
- **Decisión:**
  ...
  - **Defaults en el compose:** `${X}` sin definir llega como cadena vacía. Un `int`/`float`/lista de `Settings` con `""` revienta al arrancar, y un texto con default no vacío (`WHISPER_MODEL`, `EMAIL_FROM_NAME`) lo pierde. Las variables nuevas llevan `:-default`.
...
- **Pendiente:** el resto de servicios del compose (`celery-webhooks`, `celery-documents`, `celery-bulk`…) no se revisó contra las variables que su código lee; el test nuevo solo cubre lo gestionado aquí. Convendría extenderlo a todas las variables de `Settings`.
